...
include_once __DIR__ . "/bridge_config_helper.php";
...
class BridgePGUtil {
	...
	public function transaction_reversal(
		...
		return $result = $this->_call_bridge_api('transaction/reverse', $data);
	...
	public function recon_log(
		...
		return $result = $this->_call_bridge_api('recon/log', $data);
		/* echo json_encode($result);
		return 1; */
	}
	...
	private function _call_bridge_api($method, $data){
		...
		//$url = 'http://bridgeapi.csccloud.in/v2/' . trim($method, '/') . '/format/serialized';
...
//			CURLOPT_CUSTOMREQUEST => 'PUT',
	...
	public function decrypt_message($bridge_message){
		preg_match_all("/([^|= ]+)=([^|= ]+)/",$bridge_message, $r); 
