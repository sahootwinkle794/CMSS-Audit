...
class apply extends CI_Controller
{
	...
	*/
	...
	public function institute_page()
	{
		...
		$institute = $data['ins']; // 1stsegment
		$pro_code = $this->uri->segment(6); // 1stsegment
		...
		if($institute != '')
		{
			...
			else
			{
				...
				if( $result['status'] )
				{
					...
					$data['sidebar_data'] = $this->apply_model->apply('','get_sidebar_data');
					$data['footer_data'] = $this->apply_model->apply('','footer_information');
					...
					$data['program_data'] = $this->apply_model->apply($institute,'get_program');
					//$data['record_status'] = $this->apply_model->apply(NULL,'get_admit_card_print_status');
			...
			else
			{
				...
				if( $result['status'] )
				{
					...
					if( $result['status'] )
					{
						...
						$data['institute_data'] = $this->apply_model->apply($institute,'get_institute_data');
						$data['program_data'] = $this->apply_model->apply($program,'get_program_detail');
						$data['registration_data'] = $this->apply_model->apply($program,'get_registration_data');// print_r($data['registration_data']); return;
						...
						$data['year_wise_data_1'] = $this->apply_model->apply($program,'get_subject_wise_data_1');				
						$data['year_wise_data_2'] = $this->apply_model->apply($program,'get_subject_wise_data_2');
						$data['year_wise_data_3'] = $this->apply_model->apply($program,'get_subject_wise_data_3');
						$data['year_wise_data_4'] = $this->apply_model->apply($program,'get_subject_wise_data_4');
						...
						if( $this->input->post())
						{
							...
							else
							{
								$config = array(
					                  array(
					                     'field'   => 'center_name1',
					                     'label'   => 'center choice(1st)',
					                     'rules'   => 'trim|required'
					                  ),
					                  array(
					                     'field'   => 'center_name2',
					                     'label'   => 'center choice(2nd)',
					                     'rules'   => 'trim|required'
					                  ),
					                  array(
					                     'field'   => 'center_name3',
					                     'label'   => 'center choice(3rd)',
					                     'rules'   => 'trim|required'
					                  ),
					                  array(
					                     'field'   => 'radiogender',
					                  ...
					                  array(
					                     'field'   => 'institute_name1',
					                     'label'   => 'Institute Choice 1',
					                     'rules'   => 'trim|required'
					                  ),array(
					                     'field'   => 'institute_name2',
					                     'label'   => 'Institute choice 2',
					                     'rules'   => 'trim|required' 
					                  ),
					                  array(
					                     'field'   => 'institute_name3',
					                     'label'   => 'Institute choice 3',
					                     'rules'   => 'trim|required'
					                  ),
					                  array(
					                     'field'   => 'course_name1',
					                     'label'   => 'Course choice (1st)',
					                     'rules'   => 'trim|required'
					                  ),
					                  array(
					                     'field'   => 'course_name2',
					                     'label'   => 'Course choice (2nd)',
					                     'rules'   => 'trim|required'
					                  ),
					                  array(
					                     'field'   => 'course_name3',
					                     'label'   => 'Course choice (3rd)',
					                     'rules'   => 'trim|required'
					                  ),
					                   array(
					                     'field'   => 'txtFatherName',
							...
							{								
								$result = $this->apply_model->apply($_POST,'add_application_data_003');
								//print_r($result);die();
								if( $result['status'] )
								{
									$this->session->set_flashdata('info', $result['msg']);
				...
				if( $result['status'] )
				{
					...
					if( $result['status'] )
					{
						...
						{ 
							...
							{
								$result = $this->apply_model->apply($_POST,'update_education_details');
								//print_r($result);die();
								if( $result['status'] )
								{
									$this->session->set_flashdata('info', $result['msg']);
									
									redirect('apply/education/ins/'.$institute); 
								}
								else
								{
									$this->session->set_flashdata('error', $result['msg']);
						...
						{							
							if($this->session->userdata('reg_user_id') != '')
							{
								...
								$this->load->view('apply/education',$data);
							}
							else
							{
								redirect('apply/apply_logout');
			...
			else
			{
				...
				if( $result['status'] )
				{
					...
					if( $result['status'] )
					{
						...
						$data['guardian_data'] = $this->apply_model->apply($program,'get_guardian_data');
						$data['academic_qual_data'] = $this->apply_model->apply($program,'get_academic_qual_data');
						$data['year_wise_data_1'] = $this->apply_model->apply($program,'get_year_wise_data_1');
						
						$data['year_wise_data_2'] = $this->apply_model->apply($program,'get_year_wise_data_2');
						$data['year_wise_data_3'] = $this->apply_model->apply($program,'get_year_wise_data_3');
						$data['year_wise_data_4'] = $this->apply_model->apply($program,'get_year_wise_data_4');
						$data['allNationalities'] = $this->apply_model->apply($program,'get_nationality_data');
						$data['allCategories'] = $this->apply_model->apply($program,'get_category_data');
						$data['allReligions'] = $this->apply_model->apply($program,'get_religion_data');
						$data['allMinority'] = $this->apply_model->apply($program,'get_minority_data');
						...
						if( $this->input->post())
						{
								...
								$config = array(
									array(
					                  ...
					                  array(
					                   ...
					                   array(
					                     'field'   => 'MothersProfession',
					                     'label'   => 'Mother Profession',
					                     'rules'   => 'trim|required'
					                  ),
					                   array(
					                     'field'   => 'MothersIncome',
					                     'label'   => 'Mothers Income',
					                     'rules'   => 'trim|required'
					                  ), array(
					                     'field'   => 'co_name',
					                     'label'   => 'Name',
							...
							else
							{
								...
								$dbMessage = $result['msg'];
								$actionType = "Add Application Data";
								$this->admin_model->activity_details($dbStatus,$dbMessage,$actionType);
								//print_r($result);die();
								if( $result['status'] )
								{
									$this->session->set_flashdata('info', $result['msg']);
			...
			else
			{
				...
				if( $result['status'] )
				{
					...
					if( $result['status'] )
					{
						...
						$data['amount'] = $this->apply_model->apply($program,'amount');
	...
	*/	
	...
	public function template003_pdf() {
		$this->load->driver('session');
     	$data = array();
        $view = '';
        $order_id = 1;
		...
		$html = $this->load->view('pdf/template003_pdf', $data,  true); //load the pdf_output.php by passing our data and get all data in $html varriable.
        $pdfFilePath = "template003_pdf-" . time() . "-download.pdf";
		//actually, you can pass mPDF parameter on this load() function
        $pdf = $this->m_pdf->load();
        //generate the PDF!
        $pdf->WriteHTML($html);
