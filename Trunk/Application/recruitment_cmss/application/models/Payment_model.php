   ...
   // private $role;
    ...
    public function payment($data, $op, $stage = null) 
    {
    	switch ($op) 
    	{
			...
			//EASEBUZZ PAYMENT GATEWAY START BY NIHAL
			case 'process_payment_easebuzz':
				$dbStatus = TRUE;
				$dbMessage = "";
				...
				$this->db->where('pg_code','EASEBUZZ');
				$result = $this->db->get();
				$output_data = $result->result_array();
				foreach ($output_data as $row) 
		        {
		        ...
		        $this->db->select("T1.full_name,T1.first_name,T2.program_name,T2.program_code,T3.email_id");
				$this->db->from('applicant_master T1');
				$this->db->join('program_master T2','T1.applied_program = T2.program_code','LEFT');
				$this->db->join('applicant_reg_master T3','T1.reg_user_id = T3.reg_user_id','LEFT');
				$this->db->where('T1.reg_user_id',$reg_user_id);
				$this->db->where('T1.applied_program',$program_code);
				$this->db->where('T1.status','1');
				$result = $this->db->get();
        		...
        		$decription = "ONLINE TRANSCATION FOR  ".$program_name;
	    		$MERCHANT_KEY = $pg_parameter_values['Key'];
				$SALT = $pg_parameter_values['Salt'];
				$ENV = "prod";//"test for test enviroment or "prod" for production enviroment
				 ...
				 $postData = array (
					...
					"surl" => $pg_parameter_values['surl'],
					"furl" => $pg_parameter_values['furl'],
					"udf1" => "",
					"udf2" => "",
					"udf3" => "",
					"udf4" => "",
					...
					"udf6" => "",
					"udf7" => "",
					"address1" => $permanentaddr1,
					"address2" => $permanentaddr2,
					"city" => $permanentaddr2,
					"state" => $permanentstatecode,
					"country" => "INDIA",
					"zipcode" => $permanentpin,
					"sub_merchant_id" => ""
				  );
				$initiatepayment =  $easebuzzObj->initiatePaymentAPI($postData);
				//var_dump($initiatepayment);die;
				//return $output;
			break;
			case 'get_post_payment_data_easebuzz':
				$dbStatus = TRUE;
				$dbMessage = "";
				$reg_user_id = '';
				$institute = '';
				...
				if(isset($_POST) && $_POST["txnid"] != '' && $_POST["txnid"] != NULL){
					$order_number = $_POST["txnid"];
					...
					$query = $res_data->result_array();//print_r($query);die;
					...
					$resultresponse = $easebuzzObj->easebuzzResponse( $_POST );
					...
					$responsevalue = json_decode($resultresponse);
					$responsevalueinarray = (array) $responsevalue;
					
					$responsevalueinarray2  = (array)$responsevalueinarray['data'];
					//print_r($responsevalueinarray2);die;
					$TRANSACTIONSTATUS  = $responsevalueinarray['status'];
					if($TRANSACTIONSTATUS == TRUE || $TRANSACTIONSTATUS == 1)
					{
						...
						{
							$TRANSACTIONID = trim($_POST['txnid']);
							$AMOUNT  = trim($_POST['amount']);
							$ap_SecureHash = trim($_POST['hash']); 
						
							$merchant_secure_hash = $responsevalueinarray2['hash'];
							//echo $ap_SecureHash,"fer",$merchant_secure_hash;die;
								...
								else
								{
									...
									else
									{
										...
										else
										{
											...
											//echo $print_function;die;
											$return = $controllerInstance->$print_function();
											echo 5; var_dump($return);die;
											if($return == true)
											{
			...
			break;
			//EASEBUZZ PAYMENT GATEWAY END BY NIHAL
			
			//AIRPAY PAYMENT GATEWAY START BY NIHAL
			case 'process_payment_airpay':
				...
				return $output;
			break;
			//AIRPAY PAYMENT GATEWAY END BY NIHAL
			
			//SBI EPAY PAYMENT GATEWAY
			case 'process_payment_sbi':
				$dbStatus = TRUE;
				$dbMessage = "";
				$program_code = $this->session->userdata('admcode');
				$reg_user_id = $this->session->userdata('reg_user_id');
				...
				return array('status' => $dbStatus, 'msg' => $dbMessage,'payment_process_url' => $pg_action_url,'parameter_values'=>$pg_parameter_values,'billing_details'=>$billingDtls,'shipping_details'=>$shippingDtls);
			break;
			case 'get_post_payment_data_sbi':
				$dbStatus = TRUE;
				$dbMessage = "";
				$program_code = $this->session->userdata('admcode');
				$reg_user_id = $this->session->userdata('reg_user_id');
				...
				$this->db->from('pg_parameter_values');
				$this->db->where('pg_code','SBIEPAY');
				$this->db->where('school_code','RECINS001');
				$result = $this->db->get();
				$output_data = $result->result_array();
				foreach ($output_data as $row) 
		        {
		        ...
		        $key = $pg_parameter_values['SBIKEY'];
				$encData = $_POST["encData"];
				$merchIdVal = $_POST["merchIdVal"];
	
			    $aes = new CryptAES();
				$response = $aes->decrypt($encData,$key);
				$res = explode('|',$response);
				$order_id = $res[0];
				$pg_logged_user = $res[6];
			    $amount_paid = $res[3];
				$track_id = $res[1];
				$txn_status = $res[2]; //SUCCESS, FAIL, PENDING
				$txn_dt = $res[10];
				$payment_id = $track_id;
					...
					else
					{
						...
						else
						{
					        ...
					        if($index_no == '')
							{
								...
								$this->db->join('advertisement_master D','A.program_group = C.program_group','left');
								$this->db->where('A.program_code',$program_code);
								$result = $this->db->get();
								$output_data = $result->result_array();
								foreach ($output_data as $row1) 
								...
								//$index_no = $year_str.'/'.$key.'/'.$changed_sl_no;//YEAR+PROGRAMGROUPCODE(DRIVE)+ADVERTISEMENT SERIALNO+POST SEQUENCE NO+ SERIAL NO(FORMAT OF APPLICATION NO)
							...
							else
							{
								...
								if($return == true)
								{
									if(file_exists(DOCUMENT_UPLOAD_URL.'/DOCUMENTS/'.$program_code.'/'.$application_no.'/application_print.pdf'))
									{
										...
										$this->db->select("email_id,first_name,mid_name,last_name,pin"); 	
										$this->db->from("applicant_reg_master"); 	
										$this->db->where('reg_user_id ',$reg_user_id);
										$this->db->where('applied_program',$program_code);
										$result = $this->db->get();
			...
			break;
			//END OF SBI EPAY payment gateway
			case 'process_payment_hdfc':
				$dbStatus = TRUE;
				$dbMessage = "";
				$program_code = $this->session->userdata('admcode');
				$reg_user_id = $this->session->userdata('reg_user_id');
        		...
        		$billstatecode = '';
				$this->db->select("address_1,city_name,post_office,dm.district_name,sm.state_name,sm.state_code,pin,applicant_address.district_code,applicant_address.state_code");
				$this->db->from('applicant_address');
				$this->db->join('applicant_master appmas','applicant_address.address_ref_id=appmas.perm_address_ref_id ','LEFT');
				$this->db->join('applicant_appl_overview applov','applov.reg_user_id=appmas.reg_user_id','LEFT');
				$this->db->join('district_master dm','dm.district_code=applicant_address.district_code','LEFT');
				...
				//$billing_cust_email = $pg_parameter_values['BCE'];
        		$billingDtls = $billing_cust_name.'|'.$billing_cust_address.'|'.$permanentdistrictcode.'|'.$billstatecode.'|'.$billing_zip.'|'.$billing_country.'|'.$billing_cust_tel.'|test@test.com';
        		$delvDtls = $billing_cust_name.'|'.$billing_cust_address.'|'.$permanentdistrictcode.'|'.$billstatecode.'|'.$billing_zip.'|'.$billing_country.'|'.$billing_cust_tel.'|test@test.com';
			    ...
			    //$_SESSION['key']   = '3398330990ARVIGY';
			    //$_SESSION['key']   = $pg_parameter_values['KEY'];
				return array('status' => $dbStatus, 'msg' => $dbMessage,'payment_process_url' => $pg_action_url,'parameter_values'=>$pg_parameter_values,'billing_details'=>$billingDtls,'delivery_details'=>$delvDtls);
			break;
			case 'add_post_payment_data_hdfc':
				$dbStatus = TRUE;
				$dbMessage = "";
				$program_code = $this->session->userdata('admcode');
				$reg_user_id = $this->session->userdata('reg_user_id');
		        ...
		        $enc_key = $pg_parameter_values['ENCKEY'];
        		$this->db->select('A.institute_code,B.institute_name,logo_url');
				$this->db->from('program_master A');
				$this->db->join('institute_master B','A.institute_code = B.institute_code','INNER');
				$this->db->where('program_code',$program_code);
				...
				$enc_key;	//This is the response sent by the CCAvenue Server
				//die();
				$order_number_pay = $_POST["orderNo"];
				$rcvdString=decrypt($encResponse,$enc_key);		//Crypto Decryption used as per the specified working key.
				//die();
				$order_status="";
				$rcvdString;
				//die();
				...
				for($i = 0; $i < $dataSize; $i++) 
				{
					...
					if($i==10)	$amount_paid=$information[1];
					if($i==40)
					{
						$trans_date1 = $information[1];
						$date_one = str_replace('/', '-', $trans_date1 );
						$trans_date = date("Y-m-d H:i:s", strtotime($date_one));
					}
				}
				...
				$program_name = '';
				$this->db->select('A.institute_code,B.institute_name,logo_url,program_name');
				$this->db->from('program_master A');
				$this->db->join('institute_master B','A.institute_code = B.institute_code','INNER');
				$this->db->where('A.program_code',$program_code);
				$result = $this->db->get();
				...
				if($payment_status == 'Success' && $order_number_pay == $order_id && $amount_paid == $amount_to_pay)
				{
					$this->db->trans_start();
					$applicant_form_fee_overview = array(
						'depositdate' => $date,
				...
				{
					$dbStatus = 'Failure';
					$dbMessage = 'The transaction is failed/tempered';
					$array['print'] = 0;
					$array['payment_id'] = '';
					$array['payment_status'] = 'Failure';
					$array['paid_amount'] = '';
			...
			case 'process_payment_techprocess':
			    ...
			    $transactionRequestBean->setReturnURL($pg_parameter_values['RTU']);
