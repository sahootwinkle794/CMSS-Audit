...
//				echo $this->db->last_query();die();
			...
			case 'verify_registration_data':
				...
				$txtEmailId =isset($_POST['txtEmailId'])?$_POST['txtEmailId']:'';
				...
				if($txtEmailId != '')
				{
					$this->db->where('email_id',$txtEmailId);
				}
...
/*
							...
							*/
							...
							$query = $this->db->query("SELECT A.document_type_code,B.category_code
															...
															LEFT JOIN `course_document_setup` `B` ON `A`.`program_code` = `B`.`program_code` AND B.`course_code` IN('$course_code') 
															...
															AND A.`document_type_code` NOT IN (SELECT B.document_type_code FROM `course_document_setup` B 
																WHERE `B`.`program_code`  = '$program_code' AND B.`course_code` NOT IN('$course_code'))
...
//				die();
			...
			$html .= '<table width="100%" style="border-collapse: collapse;font-family:Arial;">
				...
				<tr >
					<br />
					<td width="50%" style="padding-left:10px;line-height:2;font-size:17px;">
						<b>Post Applied For</b> </td> 
					 <td width="5%">:</td> <td style = "font-size:17px;"width="45%"><b>'.$selectedpost.'</b></td> 
				</tr>
		</table>
				<table width="100%"  style="border-collapse: collapse;font-family:Arial;font-weight:bold;font-size:12px;padding :2px 2px 2px 2px; ">
						<tr >
