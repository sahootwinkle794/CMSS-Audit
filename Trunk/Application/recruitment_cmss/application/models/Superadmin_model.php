...
class Superadmin_model extends CI_model {
    ...
    public function importData() {
		...
		{
			$order_id = isset($data[$i]['order_id'])?$data[$i]['order_id']:'';
			...
			$query = $res->result_array();
			 /*$makeArray = array('transaction_date' => $data[$i]['transaction_date'], 'payment_date' => $data[$i]['payment_date'], 'customer_details' => $data[$i]['customer_details'], 
            					'transaction_id' => $data[$i]['transaction_id'], 'order_id' => $data[$i]['order_id'], 'bank_transaction_id' => $data[$i]['bank_transaction_id'],
            					'gross_amount' => $data[$i]['gross_amount'], 'payment_gateway_charge' => $data[$i]['payment_gateway_charge'],
            					'net_amount' => $data[$i]['net_amount'],'bank_name' => $data[$i]['bank_name'], 'transaction_status' => $data[$i]['transaction_status'], 'merchant_name' => $data[$i]['merchant_name'],
            					'payment_remark' => $data[$i]['payment_remark'],'refund_date' => $data[$i]['refund_date'], 'refund_status' => $data[$i]['refund_status'], 'refund_amount' => $data[$i]['refund_amount']);
	        */$makeArray = array('transaction_date' => isset($data[$i]['transaction_date'])?$data[$i]['transaction_date']:'',
            					'transaction_id' => isset($data[$i]['transaction_id'])?$data[$i]['transaction_id']:'',
            					 'order_id' => isset($data[$i]['order_id'])?$data[$i]['order_id']:'',
            					  'bank_transaction_id' => isset($data[$i]['bank_transaction_id'])?$data[$i]['bank_transaction_id']:'',
            					'gross_amount' => isset($data[$i]['gross_amount'])?$data[$i]['gross_amount']:'', 
            					'payment_gateway_charge' => isset($data[$i]['payment_gateway_charge'])?$data[$i]['payment_gateway_charge']:'',
            					'net_amount' => isset($data[$i]['net_amount'])?$data[$i]['net_amount']:'',
            					'bank_name' => isset($data[$i]['bank_name'])?$data[$i]['bank_name']:'');
	        if($res->num_rows() > 0) {
	        	$this->db->where('order_id', $order_id);
    ...
    public function check_user() {
	    $institute_code = $this->input->post('institute_code');// get fiest name
	    $txtUserName = $this->input->post('txtUserName');// get last name
    ...
    public function superadmin($data, $op, $stage = null) 
    {
        ...
        switch ($op) {
        	//************************** To check whether Institute is present or not *************************//
			...
			case 'program_group_data':
				$output = array("aaData" => array());
				$this->db->select("SUBSTRING_INDEX(IFNULL(MAX(program_group_code),'0')+1,'/',1) AS program_group_code");
				...
				foreach ($output_data as $aRow) 
                {
                    $program_group_code = $aRow['program_group_code'];
                }
                $year = date('Y');
				$new_sl_no = $program_group_code;
				$changed_sl_no = '';
				if($new_sl_no < 10)
				...
				$program_group_code = $changed_sl_no."/".$year;
				
				$output = array('program_group_code'=>$program_group_code);
               	return $output;
			break;
			...
			case 'add_resource': 
            	...
            	$txtResourceCode = isset($_POST['txtResourceCode'])?$_POST['txtResourceCode']:'';
            	$txtResourceName = isset($_POST['txtResourceName'])?strtoupper($_POST['txtResourceName']):'';
            	$chkIsInstruction = isset($_POST['chkIsInstruction'])?strtoupper($_POST['chkIsInstruction']):'No';
				$logged_user = $this->session->userdata('user_name');
			...
			case 'check_code_desc': 
				...
				$this->db->where('code_group',$txtCodeGroup);
				$this->db->where('record_status', 1);
			...
			case 'add_code_desc': 
            	...
            	$dbmessage = 'Data inserted successfully';
			 	...
			 	$hidCodeGroupDesc = $_POST['hidCodeGroupDesc'];//echo $hidCodeGroupDesc;die();
				...
				$this->db->select('code');
				$this->db->from('gen_code_description');
				$this->db->where('code', $txtCode);
				$result_code = $this->db->get();
				$code_arr = $result_code->result_array();
				
				if(sizeof($code_arr) == 0)
				{
					$insert_user =  $this->db->insert('gen_code_description', $new_data);//echo $hidCodeGroupDesc;die();
					if( ! $insert_user){
							$dbstatus = FALSE;
							$dbmessage = 'Error While Saving';
					}
					...
					$update_data = array(
						...
						'updated_on'					=>$date,
						'record_status'					=>1
					);//echo $hidCodeGroupDesc;
					$this->db->where('code', $txtCode);
					$this->db->update('gen_code_description', $update_data);
					if($this->db->affected_rows() == 0)
					{
						$dbstatus = FALSE;
						$dbmessage = 'Error While Saving';
			...
			case 'get_Admin_details':
				$this->db->distinct('A.employee_name');
				$this->db->select("A.employee_name,A.user_name,B.role_name,C.institute_name,A.record_status");
				$this->db->from("user_master A");
				$this->db->join('role_master B','A.role = B.role_code','left');
				$this->db->join('institute_master C','A.institute_code = C.institute_code','left');
				$result = $this->db->get();
				$output_data = $result->result_array();
				$output = array("aaData" => array());
				$slno = 1;
			...
			case 'get_applicant_details':
				...
				$sel_inst_code = $this->input->post('cmbInstitute');//echo $sel_inst_code;die();
				if($sel_inst_code !='')
				{
		  			$this->db->where("B.institute_code",$sel_inst_code);
		  		}
				$this->db->distinct('A.reg_user_id');
				$this->db->select("A.reg_user_id,A.applied_program,B.program_name,A.full_name,A.gender,B.institute_code,C.institute_name,D.appl_status");
				$this->db->from("applicant_master A");
				$this->db->join('program_master B','A.applied_program = B.program_code','left');
				$this->db->join('institute_master C','B.institute_code = C.institute_code','left');
				$this->db->join('applicant_appl_overview D','A.reg_user_id = D.reg_user_id AND D.applied_program = A.applied_program','left');
				$this->db->limit(100);
				$result = $this->db->get();
				/*echo $this->db->last_query();
				die();*/
			...
			case 'get_login':
				...
				$seldate = isset($_POST['txtLoginDate']) && $_POST['txtLoginDate'] != ''  ? date('Y-m-d',strtotime($_POST['txtLoginDate'])) : '';
				//echo $seldate; die();
				if($seldate != '')
				{
					
					$result = $this->db->select('A.user_code,A.user_ip,A.user_role,DATE_FORMAT(created_on,"%d-%m-%Y %H:%i") ');
					$this->db->from('user_activity_details as A');
					$this->db->where('A.action_type','LOGIN');
					$this->db->like('A.created_on',$seldate,'after');
...
//						die();*/
			...
			case 'get_institute_setup_data':
				$this->db->select("CONCAT(institute_name,'`',website_address) AS institute, A.institute_code, institute_type, 
							...
							admin_name,admin_user_name,institute_address,image_url,A.institute_name,A.institute_email as mail,program_view_structure");
			...
			break;
			case 'select_institute_type':
				$this->db->select('code,description');
				$this->db->from('gen_code_description');
				$this->db->where('code_group','INSTITUTE_TYPE');
				$this->db->where('record_status','1');
				$result = $this->db->get();
				$output_data = $result->result_array();
				$output = array("aaData" => array());
			...
			break;	
			case 'operation_institute_type_add':
				$ins_type = $this->input->post('txtInsType');
				$date = date('Y-m-d H:i:s', now());
				$dbStatus = "SUCCESS";
				$dbMessage = "Institute Type Added Successfully";
				$count = 0;
				$this->db->select('COUNT(*) AS cnt');
				$this->db->from('gen_code_description');
				$this->db->where('code_group','INSTITUTE_TYPE');
				$this->db->where('record_status','1');
				$this->db->where('description',$ins_type);
				$result = $this->db->get();
				$output_data = $result->result_array();
				foreach ($output_data as $aRow) 
	            {
	            ...
	            {
					$new_array = array( 
						"code"  		=>strtoupper( $ins_type ) ,
						"description"  	=>$ins_type,
						"code_group"  	=>'INSTITUTE_TYPE',
						"created_by" 	=> 'SUPADM001',
						"created_on" 	=> $date,
					);
					$sql = $this->db->insert('gen_code_description',$new_array);
					if(!$sql){
						$dbStatus = "ERROR";
						$dbMessage = "Error Inserting";
					}
				...
				{
					$dbStatus = "ERROR";
					$dbMessage = "Institute Type Already Exists";
				}
				$output = array("status"=>$dbStatus,"msg"=>$dbMessage);
			...
			case 'create_institute':
            	...
            	$dbmessage = 'Data saved successfully';
            	$image_name = "";
            	$instituteCode = 'RECINS001';
            	$type = 'RECRUITMENT';
            	$program_view_structure = 'YES';
				...
				{
					$check = mime_content_type($_FILES['fileinstitutelogo']['tmp_name']);
							if($check != 'image/jpeg' && $check != 'image/jpg' && $check != 'image/png' && $check != 'image/gif') {
								return array('status'=>false, 'msg'=>"Invalid file type");
							}
					//echo ($_FILES['fileinstitutelogo']['name']);
					$imageFileType1 = (explode(".", $_FILES['fileinstitutelogo']['name']));
					$imageFileType = end($imageFileType1);
					...
					$upload_dir = FCPATH."upload/image/logo/".$pic_name;
				...
				{
					$imageFileType1 = (explode(".", $_FILES['fileInstituteImage']['name']));
					$imageFileType = end($imageFileType1);
					$check = mime_content_type($_FILES['fileInstituteImage']['tmp_name']);
					...
					$uploads_dir = BASE_URL."upload/image/".$pic_name;
					//echo FCPATH;die();
					$upload_dir = FCPATH."upload/image/".$img_file_name;
					//echo $uploads_dir. base_url('public');
				...
				$data_institute = array( "institute_name" =>$this->input->post('institutename'),
								...
								"program_view_structure" =>$program_view_structure,
				...
				$data_appmode = array( "institute_code" =>$instituteCode,
								"online_mode" =>1,
								"offline_mode" =>0,
								"record_status" =>1,
								"created_by" => 'SUPADM001',
				...
				$data_user = array( "institute_code" =>$instituteCode,
								"user_code" =>$admin_user_name."_".$instituteCode,
				...
				$superadmin_user_code = "SUPERADMIN"."_"."$instituteCode";	
				$data_superadmin_user = array( "institute_code" =>$instituteCode,
								"user_code" =>$superadmin_user_code,
				...
				$supadm_user_code = "SUPADM"."_"."$instituteCode";	
				$data_supadm_user = array( "institute_code" =>$instituteCode,
								"user_code" =>$supadm_user_code,
								"user_name" =>"SUPADM",
								"employee_name" =>"SUPADM",
								"role" =>'SUPADM',
								"created_by" => 'SUPADM001',
								"created_on" => date('Y-m-d H:i:s', now())
							);
				...
				$logged_user_code = "$admin_user_name"."_"."$instituteCode";				
				$output = array();		
				...
				$insert_user = $this->db->insert('user_master',$data_superadmin_user);
				$insert_user = $this->db->insert('user_master',$data_supadm_user);
				...
				foreach ($result_array as $row)
				{
					...
					$data_paymode = array("institute_code" =>$instituteCode,
				...
				$query = $this->db->query("UPDATE user_master SET enc_password = SHA2(CONCAT('SUPERADMIN','#','$default_password'),512) WHERE user_code = '$superadmin_user_code' AND institute_code = '$instituteCode'");
				$query = $this->db->query("UPDATE user_master SET enc_password = SHA2(CONCAT('$admin_user_name','#','$default_password'),512) WHERE user_name = '$admin_user_name' AND institute_code = '$instituteCode'");
				$query = $this->db->query("UPDATE user_master SET enc_password = SHA2(CONCAT('SUPADM','#','$default_password'),512) WHERE user_code = '$supadm_user_code' AND institute_code = '$instituteCode'");
				...
				if($dbstatus == true)
				{
					...
					$email = $this->input->post('txtinstituteEmail');
					if($email != '')
					{
						$this->load->library('email');
						
						$body = "Congratulations!! Your Institute has been created successfully.<br /><br />
									Your Administration credentials are as follows: <br /><br />
									SuperAdmin ID : SUPERADMIN@RECINS001 <br />
									SuperAdmin Password : $default_password <br />
									Institute Admin ID : ADMIN@RECINS001 <br />
									Institute Admin Password : $default_password <br /><br />
									
									Kindly Use these credentials to proceed.
									";
						$txtSubject = "Congratulations!! Your Institute has been created successfully.";
						...
						$this->email->from($email_id, 'Recruitment Team');
						$this->email->to($email); 

						$this->email->subject($txtSubject);
						$email_content = $body;
				...
				$enc_institutecode = encrypt_decrypt('encrypt',$instituteCode);
				return array('status'=>$dbstatus,'msg'=>$dbmessage,'user_name'=>$admin_user_name,'password'=>$default_password,'enc_institute_code'=>$enc_institutecode,'institute_code'=>$instituteCode);
			break;
			...
			case 'edit_institute':
				$data_edit_institute_logo = '';
				$data_edit_institute_image = '';
				if(isset($_FILES['fileinstitutelogoEdit']['tmp_name']) && !empty($_FILES['fileinstitutelogoEdit']['tmp_name']))
				{
					$imageFileType = (explode(".", $_FILES['fileinstitutelogoEdit']['name']));
					$imageFileType = end($imageFileType);
					$check = mime_content_type($_FILES['fileinstitutelogoEdit']['tmp_name']);
					if($check != 'image/jpeg' && $check != 'image/jpg' && $check != 'image/png'  && $check != 'image/gif') {
						return array('status'=>false, 'msg'=>"Invalid file type");
					}
					...
					$uploads_dir = FCPATH."upload/image/logo/".$pic_name;
					//echo $uploads_dir. base_url('public');
					$result = move_uploaded_file($_FILES['fileinstitutelogoEdit']['tmp_name'], $uploads_dir);
					$data_edit_institute_logo = array(
								"logo_url" =>$pic_name,
							);
					}
				}
				...
				{
					$imageFileType = explode(".", $_FILES['fileInstituteImageEdit']['name']);
					$imageFileType = end($imageFileType);
					$check = mime_content_type($_FILES['fileInstituteImageEdit']['tmp_name']);
					if($check != 'image/jpeg' && $check != 'image/jpg' && $check != 'image/png'  && $check != 'image/gif') {
						return array('status'=>false, 'msg'=>"Invalid file type");
					}
					...
					//echo FCPATH."#".APPPATH."##".BASEPATH;
					//die();
					$data_edit_institute_image = array(
								"image_url" =>$img_file_name,
							);
					}
				}
				...
				$data_edit_institute = array( "institute_name" => $this->input->post('instituteeditname'),
								...
								"institute_address" =>$this->input->post('instituteeditAddress'),
				...
				if($data_edit_institute_logo != '')
				{
					$this->db->where('institute_code',$this->input->post('instituteeditcode'));
					$insert_logo = $this->db->update('institute_master',$data_edit_institute_logo);
				}
					
				if($data_edit_institute_image != '')
				{
					$this->db->where('institute_code',$this->input->post('instituteeditcode'));
					$insert_image = $this->db->update('institute_master',$data_edit_institute_image);
				}
            ...
            case 'add_image_to_institute':
            	...
            	$upload_dir = DOCUMENT_UPLOAD_URL."/institute";
            	if(!is_dir($upload_dir))
					mkdir($upload_dir,0777,true);
            	$dbstatus = TRUE;
            	$dbmessage = 'Data saved successfully';
				...
				{
					$imageFileType = explode(".", $_FILES['fileInstituteImage1']['name'])[1];
					$check = mime_content_type($_FILES['fileInstituteImage1']['tmp_name']);
					if($check != 'image/jpeg' && $check != 'image/jpg' && $check != 'image/png' && $check != 'image/gif') {
						return array('status'=>false, 'msg'=>"Invalid file type");
					}
				...
				{
					$imageFileType = explode(".", $_FILES['fileInstituteImage2']['name'])[1];
					$check = mime_content_type($_FILES['fileInstituteImage2']['tmp_name']);
					if($check != 'image/jpeg' && $check != 'image/jpg' && $check != 'image/png' && $check != 'image/gif') {
						return array('status'=>false, 'msg'=>"Invalid file type");
					}
				...
				{
					$imageFileType = explode(".", $_FILES['fileInstituteImage3']['name'])[1];
					$check = mime_content_type($_FILES['fileInstituteImage3']['tmp_name']);
					if($check != 'image/jpeg' && $check != 'image/jpg' && $check != 'image/png' && $check != 'image/gif') {
						return array('status'=>false, 'msg'=>"Invalid file type");
					}
				...
				$this->db->select("count(*) AS cnt");
				$this->db->from("institute_image_setup AS A");
				$this->db->where("institute_code",$this->input->post('cmbInstituteNameAdd'));
				//echo $this->db->last_query();
				//die();
				$res = $this->db->get();
				//echo $res1 = $this->db->last_query();
            ...
            case 'edit_added_image_to_institute':
				...
				{
					$imageFileType = explode(".", $_FILES['fileInstituteImageEdit1']['name']);
					$imageFileType = end($imageFileType);
					$check = mime_content_type($_FILES['fileInstituteImageEdit1']['tmp_name']);
					if($check != 'image/jpeg' && $check != 'image/jpg' && $check != 'image/png' && $check != 'image/gif') {
						return array('status'=>false, 'msg'=>"Invalid file type");
					}
				...
				{
					$imageFileType = explode(".", $_FILES['fileInstituteImageEdit2']['name']);
					$imageFileType = end($imageFileType);
					$check = mime_content_type($_FILES['fileInstituteImageEdit2']['tmp_name']);
					if($check != 'image/jpeg' && $check != 'image/jpg' && $check != 'image/png' && $check != 'image/gif') {
						return array('status'=>false, 'msg'=>"Invalid file type");
					}
				...
				{
					$imageFileType = explode(".", $_FILES['fileInstituteImageEdit3']['name']);
					$imageFileType = end($imageFileType);
					$check = mime_content_type($_FILES['fileInstituteImageEdit3']['tmp_name']);
					if($check != 'image/jpeg' && $check != 'image/jpg' && $check != 'image/png' && $check != 'image/gif') {
						return array('status'=>false, 'msg'=>"Invalid file type");
					}
			...
			case 'get_institute_application_mode':
                ...
                foreach ($query as $row) 
                {
					...
					$row1[0] = $sl_no;
					$row1['sl_no'] = $sl_no;
					$row1[1] = $row['institute_code']; 
					$row1['institute_code'] = $row['institute_code'];
					$row1[2] = $row['institute_name']; 
					$row1['institute_name'] = $row['institute_name'];
					$row1[3] = $row['online_mode'];
					$row1['online_mode'] = $row['online_mode'];
					$row1[4] = $row['offline_mode'];
					$row1['offline_mode'] = $row['offline_mode'];
					$row1[5] = $row['online_app_mode'];
					$row1['application_mode'] = $row['online_app_mode'];
					if($row['offline_app_mode'] != '')
					{
					$row1[5] = $row1[5].", ".$row['offline_app_mode'];
					$row1['application_mode'].", ".$row['offline_app_mode'];
					}
			...
			case 'get_institute_payment_mode':
                ...
                $this->db->limit($iDisplayLength, $iDisplayStart);
                $this->db->select("A.institute_code,B.institute_name,GROUP_CONCAT(CASE WHEN A.record_status = '0' THEN NULL ELSE payment_mode END) AS payment_mode,
						GROUP_CONCAT(CASE WHEN A.record_status = '0' THEN NULL ELSE description END) AS payment_mode_name");
				$this->db->from("payment_mode_setup AS A");
				$this->db->join("institute_master AS B","A.institute_code = B.institute_code","left");
				$this->db->join("gen_code_description AS C","A.payment_mode = C.code","left");
				$this->db->where("C.code_group","PAYMENT_MODE");
				$this->db->where("B.record_status","1");
				$this->db->group_by('A.institute_code'); 
				
				//die();
				$res = $this->db->get();
				//echo $res1 = $this->db->last_query();die();
                $query = $res->result_array();
                $output = array("aaData" => array());
                /*----FOR PAGINATION -----*/
                if ($search['value'] != '') 
				...
				$this->db->select("A.institute_code,institute_name,GROUP_CONCAT(payment_mode) AS  payment_mode,
					GROUP_CONCAT(description,'@',B.record_status) AS description");
				$this->db->from("payment_mode_setup AS A");
				$this->db->join("institute_master AS B","A.institute_code = B.institute_code","left");
				$this->db->join("gen_code_description AS C","A.payment_mode = C.code AND C.code_group = 'PAYMENT_MODE'","left");
				$this->db->group_by('A.institute_code'); 
			...
			*/
			...
			break;
			case 'get_menu_design':
				$cmbrole = $data['cmbrole'];
				$this->db->select("role_name,menu_id, link_text, link_url, parent_id, sl_no, 
								has_child,is_last_child,icon_class");
				$this->db->from('menu_master');
				$this->db->join('role_master','menu_master.role_code = role_master.role_code','inner');
				$this->db->where('menu_master.role_code',$cmbrole);
				$this->db->where('menu_master.record_status','1');
				$this->db->order_by('sl_no');
				$result = $this->db->get();
				return $result1 = $result->result_array();
			break;	
			...
			case 'get_institute' :
				...
				$this->db->where('i.record_status',1);
				$this->db->where('i.institute_code!=','EDUSOLS');
				$result = $this->db->get();
				//echo $this->db->last_query();
				$output_data = $result->result_array();
				$output = array("aaData" => array());
			...
			break;
			case 'get_user_data' :
				$this->db->select("i.employee_name,i.user_name, i.role,i.image_file_name,i.record_status, i.user_code");
			...
			case 'add_menu':
				...
				$chkHasChild = ($this->input->post('chkHasChild')!= null)?$this->input->post('chkHasChild'):'';
			...
			case 'edit_menu':
                ...
                $sql = $this->db->update('menu_master', $new_data);
                /*echo $this->db->last_query();
                die();*/
                if ($sql)
                    return array('status' => $dbstatus, 'msg' => $dbmessage);
			...
			case 'copy_menu':
				...
				{
	                	$dbstatus = 'FAILURE';
            			$dbmessage = 'Data already exists for this role. Please delete all the data to copy from other role.';
					
				}
				return array('dbstatus' => $dbstatus, 'dbMessage' => $dbmessage);
			break;
			/**
			...
			*/
					...
					if($cmbInstituteforExamAdd != "")
					{
	            		...
	            		for($j=0;$j<$cmbExamCenterAddSizeof;$j++)
	            		{
	            			if($cmbExamCenterAdd[$j] != "multiselect-all")
							{
								...
								$new_data = array(
									'institute_code' 			=>$cmbInstituteforExamAdd,
								...
								$this->db->where("institute_code",$cmbInstituteforExamAdd);
			...
			case 'get_program_menu_generic_setup1' :
				$this->db->select("menu_code,link_text,link_url,is_new_window,is_document_upload,CASE WHEN record_status = 'Active' THEN '1' ELSE '0' END AS record_status,prog_sl_no,id");
			...
			case 'add_program_menu': 
				$dbstatus = TRUE;
            	$dbmessage = 'Data saved successfully';
            	$txtStandardCode = isset($_POST['txtStandardCode'])?$_POST['txtStandardCode']:'';
			 	$txtMenuCode = isset($_POST['txtMenuCode'])?$_POST['txtMenuCode']:'';
			 	$txtLinkText = isset($_POST['txtLinkText'])?$_POST['txtLinkText']:'';
			 	$txtLinkURL = isset($_POST['txtLinkURL'])?$_POST['txtLinkURL']:'';
			 	$cmbNewWindow = isset($_POST['cmbNewWindow'])?$_POST['cmbNewWindow']:'';
			 	$cmbDocumentUpload = isset($_POST['cmbDocumentUpload'])?$_POST['cmbDocumentUpload']:'';
			 	$cmbRecordStatus = isset($_POST['cmbRecordStatus'])?$_POST['cmbRecordStatus']:'';
			 	$txtProgramMenuSlno = isset($_POST['txtProgramMenuSlno'])?$_POST['txtProgramMenuSlno']:'';
				$logged_user = $this->session->userdata('user_name');
			 	$op_type = 'add_code';
			 	date_default_timezone_set('Asia/Kolkata');
				$date = date('Y-m-d H:i:s', time());
			...
			case 'check_program_group_add': 
				...
				return $output; 
			break;
			case 'check_program_group_edit': 
				$dbstatus = TRUE;
            	$dbmessage = 'Data saved successfully';
            	
			 	$txtProgramGroupCodeEdit = $_POST['txtProgramGroupCodeEdit'];
			 	
				$this->db->select('program_group_code');
				$this->db->from('program_group_master');
				$this->db->where('program_group_code',$txtProgramGroupCodeEdit);
				$res = $this->db->get();
				$query = $res->result_array();
				foreach($query as $row)
				{
			...
			$this->db->select("program_group_code,program_group_name,DATE_FORMAT(application_last_date,'%d-%m-%Y') AS application_last_date,
			        CASE WHEN record_status = 1 THEN 'ACTIVE' 
			...
			case 'add_program_group': 
				...
				'application_last_date' 		=>date('Y-m-d', strtotime($txtLastDate)),
				'record_status' 				=>$cmbRecordStatus,
				'created_by'					=>'SUPADM001',
				'created_on'					=>$date
				);
			...
			case 'edit_program_group': 
            	...
            	$dbmessage = 'Data saved successfully';
			 	...
			 	$txtLastDateEdit = $_POST['txtLastDateEdit'];
				...
				'program_group_code' 			=>$txtProgramGroupCode,
				'program_group_name' 			=>$txtProgramGroupName,
				'application_last_date' 		=>date('Y-m-d', strtotime($txtLastDateEdit)),
				'record_status' 				=>$cmbRecordStatus,
				'updated_by'					=>'SUPADM001',
				'updated_on'					=>$date
				);
			...
			case 'edit_document': 
            	...
            	$dbmessage = 'Data saved successfully';
			 	...
			 	$txtDocumentTypeEdit = $_POST['cmbDocumentTypeEdit'];
			...
			case 'add_instruction': 
	            ...
	            {
					$this->db->where('page_code',$cmbPageCode);
					$insert_user =  $this->db->update('instruction_setup', $new_data);
				}
				else
				{
			...
			break;
			case 'check_religion' :
				$txtReligionCode = $_POST['txtReligionCode'];//echo $txtCasteCode;
				$this->db->select("religion_code");
				$this->db->from('religion_master');
				$this->db->where('religion_code',$txtReligionCode);
				$result = $this->db->get();//echo $this->db->last_query();die();
				$output_data = $result->result_array();
				foreach($output_data as $row)
				{
			...
			break;
			case 'check_religion_edit' :
				$txtReligionCodeEdit = $_POST['txtReligionCodeEdit'];//echo $txtCasteCode;
				$this->db->select("religion_code");
				$this->db->from('religion_master');
				$this->db->where('religion_code',$txtReligionCodeEdit);
				$result = $this->db->get();//echo $this->db->last_query();die();
				$output_data = $result->result_array();
				foreach($output_data as $row)
				{
			...
			case 'check_instruction' :
				$cmbPageCode = $_POST['cmbPageCode'];//echo $txtCasteCode;
			...
			case 'check_instructionEdit' :
				$cmbPageCodeEdit = $_POST['cmbPageCodeEdit'];//echo $cmbPageCodeEdit;
				$this->db->select("page_code");
				$this->db->from('instruction_setup');
			...
			case 'check_sms_provider' :
				...
				foreach ($output_data as $aRow) 
	            {
	           	...
	           	return $output;
			break;
			case 'check_email_mail_type' :
				$provider_name = $_POST['mailtype'];
				$institute_code = $this->session->userdata('institute_code');
				//$provider_name = $provider_name.'_'.$institute_code;
				
				$this->db->select("email_type");
				$this->db->from('email_setup');
				$this->db->where('email_type',$provider_name);
				$this->db->where('record_status','1');
				$result = $this->db->get();
				//echo $this->db->last_query();
				$output_data = $result->result_array();
			...
			case 'add_sms_provider': 
				...
				$this->db->select('COUNT(*) AS cnt');
				$this->db->from('sms_provider_setup');
				$this->db->where('provider_name',$instsmsProvider);
				$res = $this->db->get();
				//echo $this->db->last_query();
				$query = $res->result_array();
				foreach($query as $row)
			...
			case 'edit_provider': 
            	...
            	//print_r($_POST);
			 	...
			 	$id = $_POST['hidprovideridEdit'];//echo $id;die();
				...
				$sql = $this->db->update('sms_provider_setup', $new_data);
				if(!$sql){
						$dbstatus = FALSE;
						$dbmessage = 'Error While Saving';
				}
				//echo $this->db->last_query().' '.$id;die();
				$output = array('status'=>$dbstatus,'msg'=>$dbmessage);
			...
			case 'get_sms_setup' :
				$this->db->select("sms_type,subject,content,SUBSTRING_INDEX(provider_name,'_',1) AS provider_name,status,status as status_image,sms_setup_id, provider_name AS prov_name");
			...
			case 'add_sms': 
				...
				$this->db->select('COUNT(*) AS cnt');
				$this->db->from('sms_setup');
				$this->db->where('sms_type',$cmbsmsType);
				$res = $this->db->get();
				//echo $this->db->last_query();
				$query = $res->result_array();
				foreach($query as $row)
			...
			case 'edit_sms': 
				...
				$sql = $this->db->update('sms_setup', $new_data);
				if(!$sql){
						$dbstatus = FALSE;
						$dbmessage = 'Error While Saving';
				}
			...
			case 'add_email_provider': 
            	...
            	$dbmessage = 'Data saved successfully';
			 	...
			 	$txtProvider = $_POST['txtproviderEmail'];
			...
			case 'edit_email_provider': 
				...
				$sql = $this->db->update('email_provider_setup', $new_data);
				if(!$sql){
						$dbstatus = FALSE;
						$dbmessage = 'Error While Saving';
				}
			...
			case 'add_email': 
				...
				$this->db->select("count(*) as cnt");
				$this->db->from('email_setup');
				$this->db->where('email_type',$cmbsmsType);
				//$this->db->where('record_status','1');
				$result = $this->db->get();
				//echo $this->db->last_query();
				$output_data = $result->result_array();
			...
			case 'edit_email': 
				...
				$sql = $this->db->update('email_setup', $new_data);
				if(!$sql){
						$dbstatus = FALSE;
						$dbmessage = 'Error While Saving';
				}
			...
			case 'add_user_program' :
				...
				{
					$dbStatus = TRUE;
					$dbMessage = 'Data assigned successfully';
				}
				else
				{
					$dbStatus = FALSE;
			...
			*/
			...
			case 'delete_country':  	
				$dbstatus = TRUE;
            	$dbmessage = 'Data deleted successfully';
			 	$countryID = $_POST['txtCountryCodeEdit'];
				$output = array();
				$new_data = array(
				'record_status'					=>0
				);
				$this->db->where('country_code', $countryID);
			...
			//State
			case 'select_state' :
			$this->db->select("country_name,state_code,state_name,state_master.id, state_master.country_code");
			$this->db->from('state_master');
			...
			$this->db->select("country_master.country_name,state_master.state_name,district_code,district_name,district_master.id,state_master.state_code,country_master.country_code ");
			$this->db->from('district_master');
			...
			$this->db->select("qualification_code,qualification_name,record_status,record_status as status");
			...
			case 'insert_center': 
            	...
            	$centrecode = isset($_POST['txtCenterCode'])?strtoupper($_POST['txtCenterCode']):'';
			 	$centrename = isset($_POST['txtCenterName'])?$_POST['txtCenterName']:'';
			 	$cmbCenterStatus = isset($_POST['cmbCenterStatus'])?$_POST['cmbCenterStatus']:'';
			 	$cmbRecordStatus = 1;
				$logged_user = $this->session->userdata('user_name');
			...
			case 'update_centre': 
				...
				'institute_code' 				=>$institute_code,
				'record_status' 				=>$cmbCenterStatusEdit,
				'updated_by'					=>'SUPADM001',
				'updated_on'					=>$date
				);
				$this->db->where('exam_centre_code',$id );
				$query = $this->db->update('exam_centre', $new_data);
				if(!$query){
						$dbstatus = FALSE;
						$dbmessage = 'Error While Saving';
				}
			...
			case 'check_duplicate_country_edit': 
            	...
            	$dbmessage = 'Data saved successfully';
			 	...
			 	$txtCountryCodeEdit = $_POST['txtCountryCodeEdit'];
			 	$this->db->select('country_code');
				$this->db->from('country_master');
				$this->db->where('country_code',$txtCountryCodeEdit);
				$this->db->where('record_status',1);
				$res = $this->db->get();
				$query = $res->result_array();
				foreach($query as $row)
			...
			case 'get_file_name' :
				...
				$this->db->select("resource_code, resource_name");
				$this->db->from('resource_master');
				$this->db->like('resource_code','temp');
				$this->db->where('record_status',1);
				/*$this->db->select("template_code,template_name,template_description,file_name,id");
				$this->db->from('form_template_master');
				$this->db->where('record_status','1');*/
			...
			case 'check_duplicacy_menu_code': 
				...
				$this->db->select('menu_code');
				$this->db->from('program_menu_master');
				$this->db->where('menu_code',$txtCode);
				$res = $this->db->get();
				$query = $res->result_array();
				foreach($query as $row)
				{
			...
			case 'select_file_name':
				...
				$this->db->select("resource_code, resource_name");
				$this->db->from('resource_master');
				$this->db->like('resource_code','reg');
			...
			case 'delete_registration_template':  
				...
				if(in_array($templateID,$template_code)){
					$dbstatus = "Error";
            		$dbmessage = 'Template has been used in other program.First unassign this template then delete';
					//echo"dsffsdf";die()
				}
				else
				{
					...
					if($this->db->affected_rows()){
						$dbstatus = true;
						$dbmessage ='Template has been deleted successfully.';
					}
					if(!$this->db->affected_rows()){
						$dbstatus = FALSE;
						$dbmessage = 'Error While Deleting';
			...
			case 'get_roundWiseStatus':
			
				$program = $_POST['program_code'];
				$this->db->select("program_code,round_no,log");
				$this->db->from('admitcard_log_details');
			...
			case 'check_publish_status':
				...
				$where1 = "round_no = (SELECT MAX(round_no) AS round_no FROM admitcard_log_details ORDER BY id DESC LIMIT 1)";
				$log1 = 'NO';
				$this->db->select("CASE WHEN log = 'Add Result' THEN 'NO' ELSE log END AS log1");
				$this->db->from('admitcard_log_details');
				$this->db->where('program_code',$program);
			...
			case 'change_publish_status':
				$program = $_POST['program_code'];
				$publish_status = $_POST['status'];
				$logged_user = $this->session->userdata('user_name');
				date_default_timezone_set('Asia/Kolkata');
        		$date = date('Y-m-d H:i:s', now());
        		$dbStatus = 'SUCCESS';
        		$dbMessage = 'Status Changed Successfully';
				
				if($publish_status == 'End') // to continue admit card generation
				{
					$this->db->select("MAX(round_no) AS round_no");
					$this->db->from('admitcard_log_details');
					$this->db->where('program_code',$program);
					$this->db->where('log',$publish_status);
					$result = $this->db->get();
					foreach($result->result_array() AS $row1)
					{
						$round_data = $row1['round_no'];
					}
					$round_data1 = $round_data + 1;
					
					$this->db->select("appl_no,program_code,roll_no,round_no,reg_user_id");
					$this->db->from('final_result_data');
					$this->db->where('program_code',$program);
					...
					foreach($result->result_array() AS $row1)
					{
						...
						$this->db->where("program_code",$row1['program_code']);
						$this->db->where("roll_no",$row1['roll_no']);
						$this->db->where("round_no",$row1['round_no']);
						$this->db->where("reg_user_id",$row1['reg_user_id']);
						$delete_final_result_data = $this->db->delete('final_result_data');
						if(!$delete_final_result_data)
						{
							$dbStatus = FALSE;
							$dbMessage = 'Error deleting final_result_data';
						}
				...
				else // to end admit card generation
				{
					$this->db->select("MAX(round_no) AS round_no");
					$this->db->from('admitcard_log_details');
					$this->db->where('program_code',$program);
					...
					foreach($result->result_array() AS $row1)
					{
						...
						$sql = $this->db->insert('final_result_data',$new_data1);
						if(!$sql){
							$dbStatus = "ERROR";
							$dbMessage = "Error Inserting final_result_data";
						}
						
						$round_data1 = $row1['round_no'] + 1;
						
						$this->db->where("program_code",$row1['program_code']);
						$this->db->where("round_no",$round_data1);
						$this->db->where("reg_user_id",$row1['reg_user_id']);
						$delete_final_result_data = $this->db->delete('admitcard_round_data');
						if(!$delete_final_result_data)
						{
							$dbStatus = FALSE;
							$dbMessage = 'Error deleting final_result_data';
				...
				if($application_status != 'Verified')
				{
					...
					if($query && $query1 && $query2)
					{
						...
						if($query2)
						{
							...
							$this->session->set_userdata('appl_no', $hidApplNoEdit);
							...
							if($return == true)
							{
								if(file_exists(DOCUMENT_UPLOAD_URL.'/DOCUMENTS/'.$hidApplProgramiEdit.'/'.$hidApplNoEdit.'/application_print.pdf'))
								{
									$this->db->trans_complete();
									$dbmessage = "Success";
									$_SESSION['reg_user_id'] = '';
			...
			case 'add_payment_gateway_master': 
				...
				else
				{
					if($filelogo != '')
					{
						...
						$uploaddir = DOCUMENT_UPLOAD_URL."/images/logo/";//echo $uploaddir;
						if(!is_dir($uploaddir))
							mkdir($uploaddir,0777,true);
			...
			case 'add_pg_parameter_values': 
				...
				foreach($institute_details_arr as $key => $value)
				{
					...
					if( $error_count > 0){
						$dbstatus = "Error";
						$response = "Duplicate entry of PG name & Parameter Name for Organization .\n";
					}
					else
					{
						$new_data = array(
			...
			case 'reset_password':
            	...
            	$user_code = $this->input->post('user_code');//echo $user_code;die();
            	
				$this->db->query("UPDATE user_master 
								SET enc_password = SHA2(CONCAT(user_name,'#','password'), 512), updated_by = 'SUPADM001', updated_on = '".$date."' 
								WHERE user_code = '".$user_code."'");
				//echo $this->db->last_query().$this->db->affected_rows();die();
				if($this->db->affected_rows() == 0){
					$dbstatus = false;
