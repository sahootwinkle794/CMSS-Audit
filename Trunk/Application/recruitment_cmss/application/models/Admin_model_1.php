<?php
class Admin_model_1 extends CI_model {
		...
		*/
        switch ($op) {
				      	...
				      	<div class="info-box">
				        	...
				        	<div class="info-box-content">
				          		<a href="javascript:void(0);" onclick="show_pay_detail()">
				          			...
				          			<span class="info-box-number" style="display: block; font-size: 15px; font-weight: normal;">Total Paid</span>
				          		</a>	
				        	</div>
				        </div>
				    </div>
			...
			*/
			...
			break;
			case 'CHKDUCPLICATE_program_name':
				$dbstatus = '';
				$institute_code = $this->session->userdata('institute_code');
				date_default_timezone_set('Asia/Kolkata');
        		$date = date('Y-m-d', now());
				if($_POST['validateprogramcode'])
				{
					...
					$this->db->select('COUNT(program_name) AS program_name');
					$this->db->from('program_master');
					$this->db->where('program_name',$txtProgramName);
					$this->db->where('record_status','1');
					$result = $this->db->get();
					//echo $this->db->last_query();
					$output_data = $result->result_array();
					...
					foreach ($output_data as $aRow) 
		            {
						$output['aaData'][] = $aRow['program_name'];
						$dbstatus = $aRow['program_name'];
			...
			case 'get_copyformdata':
				...
				$this->db->where('record_status','1');
				$this->db->where('institute_code',$institute_code);
				$this->db->not_like('program_code', 'PROG_');
				$this->db->order_by(' A.sl_no');
				$result = $this->db->get();
				$output_data = $result->result_array();
				//echo $this->db->last_query();
			...
			case 'insert_copydata': 
				...
				$txtOnlineTransactionNo = 1;
				$txtOmrNo = 1;
				$txtStartdate = isset($_POST['txtStartdateCopy'])?$_POST['txtStartdateCopy']:'';
				$txtAdmitCardStartdate = isset($_POST['txtAdmitCardStartdateCopy'])?$_POST['txtAdmitCardStartdateCopy']:'';
				$txtAdmitCardEnddate = isset($_POST['txtAdmitCardEnddateCopy'])?$_POST['txtAdmitCardEnddateCopy']:'';
				$txtAgeStartdate = isset($_POST['txtAgeStartdateCopy'])?$_POST['txtAgeStartdateCopy']:'';
				$txtAgeEnddate = isset($_POST['txtAgeEnddateCopy'])?$_POST['txtAgeEnddateCopy']:'';
				$txtEnddate = isset($_POST['txtEnddateCopy'])?$_POST['txtEnddateCopy']:'';
				$txtAppStartdate = $_POST['txtAppStartdateCopy'];
				$txtAppEnddate = $_POST['txtAppEnddateCopy'];
				$program_code = $txtProgramCode."_".$institute_code;
				$logged_user_code = "$logged_user"."_"."$institute_code";
				$txtYear = date('Y');
			...
			case 'get_dms_modal_data' :
				...
				$this->db->select('exam_center_code,exam_vanue_code,IFNULL(ap.record_status, "0") AS record_status, `as`.`template_code`, E.template_code,`E`.`template_name`');
				...
				foreach($output_data as $row)
				{
					...
					$admit_template_code = $row['template_code'];
	            ...
	            if(sizeof($form_documents) >= 1)
				{
					...
					{
						$admit_temp_name = "admit_card_".$admit_template_code;
						$exam_vanue_new = str_replace('/','_',$exam_vanue);
						$html .= '<br />
							<a href="'.$base_url.'admin/'.$admit_temp_name.'/'.$exam_center_code.'/'.$exam_vanue_new.'/'.$program_code.'/'.$reg_user_id.'/'.$institute_code.'" target="_blank">&raquo;  <span style="color:red">	
							 Admit Card<br/>
						    </a>
							';		
					}
...
//				die();
		...
		$html .= '</table>
			...
			case 'update_multiple_cutoff_vacancy' :
				$dbStatus = "SUCCESS";
				$dbMessage = "Assigned Successfully";
				$dbError = "";
				$institute_code = $this->session->userdata('institute_code');
				...
				$category_codes = $_POST['category_codes']?$_POST['category_codes']:'';
				//print_r($category_codes);
				$cutoff_mark = $_POST['cutoff_mark']?$_POST['cutoff_mark']:'';
				$arr_program_code = call_user_func_array('array_merge', $program_codes);
				...
				for($i=0;$i<sizeof($arr_program_code);$i++)
				{
					for($j=0;$j<sizeof($category_codes);$j++)
					{
						...
						{
							$this->db->select("count(program_code) AS program_code");
							$this->db->from('category_cutoff_details');
							$this->db->where('program_code',$arr_program_code[$i]);
							$this->db->where('category_code',$category_codes[$j]);
							...
							foreach ($output_data as $aRow) 
		            		{
								...
								if($result >= 1){
									if($arr_program_code[$i] != "multiselect-all")
									{
										$update_data = array(
											'category_code'					=>$category_codes[$j],
											'cutoff_mark' 					=>$cutoff_mark[$j],
											'updated_by'					=>$user,
											'updated_on'					=>$date
										);
										$this->db->where('program_code',$arr_program_code[$i] );
										$this->db->where('category_code',$category_codes[$j] );
										$sql = $this->db->update('category_cutoff_details', $update_data);
										//echo $this->db->last_query();
										if(!$sql){
											$dbStatus = "ERROR";
											$dbMessage = "Error Inserting";
									...
									if($arr_program_code[$i] != "multiselect-all"){
										$sql = $this->db->insert('category_cutoff_details', $new_data);
										//echo $this->db->last_query();
										if(!$sql){
											$dbStatus = "ERROR";
											$dbMessage = "Error Inserting";
			...
			case 'get_vacancy_single':
				...
				foreach ($output_data as $aRow) 
	            {
	           	...
	           	return $output;
			break;
			case 'assign_single_cutoff_vacancy' :
				$dbStatus = "SUCCESS";
				$dbMessage = "Updated Successfully";
				$dbError = "";
			...
			case 'get_applicant_details_scrutiny' :
				...
				if($status != ''){
					...
					$this->db->where("(B.scrutiny_status is NULL or B.scrutiny_status='')");
				}	
				$this->db->select("SUBSTRING(C.appl_no,-10) as appl_no,A.reg_user_id,A.reg_user_id AS mobile_no,B.full_name,case when B.scrutiny_status='Valid' then 'Approved' when B.scrutiny_status='Invalid' then 'Rejected' when B.scrutiny_status is null then 'Applied' else B.scrutiny_status end as scrutiny_status ,B.scrutiny_remark,C.index_no,date_format(C.updated_on,'%d-%m-%Y %H:%i') as submission_date,date_format(A.updated_on,'%d-%m-%Y %H:%i') as scrutinized_date");
				$this->db->from('applicant_master B');
				$this->db->join('applicant_reg_master A','A.reg_user_id = B.reg_user_id','inner');
				$this->db->join('applicant_appl_overview C ','A.reg_user_id = C.reg_user_id AND B.applied_program = C.applied_program','inner');
				$this->db->where('B.applied_program',$program);
			...
			case 'get_scrutiny_dates' :
				$institute_code = $this->session->userdata('institute_code');
				date_default_timezone_set('Asia/Kolkata');
        		$date = date('Y-m-d', now());
        		$program =  isset($_POST['program_code'])? $_POST['program_code']:'';
        		$scrutiny_status = '0';
        		
        		$this->db->select("scrutiny_start_date,scrutiny_end_date");
				$this->db->from('program_master');
				...
				foreach ($output_data as $aRow) 
	            {
	            	$scrutiny_end_date = $aRow['scrutiny_end_date'];
	            	$scrutiny_start_date = $aRow['scrutiny_start_date'];
	            }
	           	return array('scrutiny_start_date' => $scrutiny_start_date, 'scrutiny_end_date' =>$scrutiny_end_date);
			...
			case 'get_program_group_scrutiny_applicants' :
				...
				$this->db->distinct("P.program_group");
				$this->db->select("P.program_group,program_group_name");
				$this->db->from('program_master P');
				$this->db->join('user_program_mapping U','P.program_code = U.program_code ','inner');
				$this->db->join('program_group_master A ','P.program_group = A.program_group_code','inner');
				$this->db->where('P.institute_code',$institute_code);
				$this->db->where('U.institute_code',$institute_code);
				$this->db->where('U.user_code',$user);
				$this->db->where('P.publish_status','YES');
			...
			case 'get_reject_sbi_applnts':
				...
				return $output;
			break;
			case 'disqualify_scrutiny_applicants1': 
			...
			case 'disqualify_scrutiny_applicants': 
				...
				$this->db->join('applicant_master am','arm.reg_user_id = am.reg_user_id and arm.applied_program =am.applied_program','INNER');
				...
				if($dbStatus == "SUCCESS")
				{
					...
					$this->db->where('pes.institute_code',$institute_code);
					...
					if($row_count > 0)
					{
					     ...
					     $this->email->from('svnirtar.pget2019@gmail.com', 'NIRTAR ADMIN');
					     $this->email->to($email_id);
			...
			case 'qualify_scrutiny_applicants': 
				...
				$program =  isset($_POST['program'])? $_POST['program']:'';
				$reg_user_id =  isset($_POST['reg_user_id'])? $_POST['reg_user_id']:'';
				$taRemark =  isset($_POST['taRemark'])? $_POST['taRemark']:'';//echo $taRemark;die();
				...
				{
					$full_name = '';
					$this->db->select('*,am.full_name');
					$this->db->from('applicant_reg_master arm');
					$this->db->join('applicant_master am','arm.reg_user_id = am.reg_user_id and arm.applied_program =am.applied_program','INNER');
					$this->db->join('applicant_appl_overview aao','am.reg_user_id = aao.reg_user_id and am.applied_program =aao.applied_program','INNER');
					$this->db->join('course_master cm','am.master_name = cm.course_code','INNER');
					$this->db->where('arm.reg_user_id',$reg_user_id);
					$this->db->where('arm.applied_program',$program);
					$result_output = $this->db->get();
					//echo $this->db->last_query();die();
			...
			case 'admitcard_auto_assign' :
				...
				$program_code = $_POST['cmbAutoProgram'];
            	$round_data = isset($_POST['cmbAutoRound']) ? $_POST['cmbAutoRound'] : '';
            	$dt = date('Y-m-d',strtotime($_POST['txtAutoApplDate']));
            	$where2 = "(B.updated_on <= '$dt' OR B.created_on <= '$dt')";
				$where1 = "B.appl_no NOT IN (SELECT appl_no FROM admitcard_published WHERE applied_program='$program_code' AND round_no = '$round_data')";
            	if($round_data == '1')
					{
						
						$this->db->select("A.full_name, B.appl_no, B.form_no, A.reg_user_id, B.updated_on, B.created_on");
						$this->db->from('applicant_master A');
					...
					else
					{
						...
						$where1 = "B.appl_no NOT IN (SELECT appl_no FROM admitcard_published WHERE applied_program='$program_code'
							AND round_no = '$round_data')";
						$this->db->select("A.full_name, B.appl_no, B.form_no, A.reg_user_id, B.updated_on, B.created_on");
						$this->db->from('applicant_master A');
						$this->db->join('applicant_appl_overview B','A.applied_program = B.applied_program AND A.reg_user_id = B.reg_user_id','inner');
						$this->db->join('admitcard_round_data G','B.appl_no = G.appl_no ','inner');
			...
			case 'centre_address_admit_setup' :
				...
				if($exam_centre_code == '')
				{
					$this->db->select("A.program_code,A.exam_center_code,A.round,A.exam_vanue,A.capacity,
										...
										C.exam_centre_name,template_code,A.exam_shift,A.reporting_time,A.exam_start_time,A.examination_date");
					$this->db->from('admitcard_setup A');
			...
			case 'add_centre': 
				...
				$cmbSession = $_POST['cmbSession'];
				$txtDate = $_POST['txtDate'];
				$txtStartTime = $_POST['txtStartTime'];
				$txtReportingTime = $_POST['txtReportingTime'];
					...
					foreach ($output_data as $aRow1) 
            		{
						...
						if($result >= 1)
						{
							$update_data = array(
								...
								'exam_shift' 					=>$cmbSession,
								'reporting_time' 				=>$txtReportingTime,
								'exam_start_time' 				=>$txtStartTime,
								'examination_date' 				=>date('Y-m-d', strtotime($txtDate)),
			...
			case 'edit_centre': 
				...
				$cmbSession = $_POST['cmbSessionEdit'];
				$txtDate = $_POST['txtDateEdit'];
				$txtStartTime = $_POST['txtStartTimeEdit'];
				$txtReportingTime = $_POST['txtReportingTimeEdit'];
			...
			case 'select_meritDetails' :
				...
				//$result1 = $this->db->get();
				$this->db->distinct("B.appl_no");
				$this->db->select("D.full_name,omr_no,B.appl_no,A.value,E.round_no");
				$this->db->from('admitcard_published B');
				$this->db->join('admitcard_round_data E','B.omr_no = E.roll_no','left');
				$this->db->join('applicant_exam_result A','A.applicant_id = B.omr_no AND A.program_code = B.applied_program AND A.round_no = '.$round_data.'','left');
				$this->db->join('applicant_appl_overview C','B.appl_no = C.appl_no AND B.applied_program = C.applied_program','left');
			...
			case 'ADD_Result' :
				...
				{
					$this->db->select("D.full_name,D.category,omr_no,B.applied_program");
					$this->db->from('admitcard_published B');
					$this->db->join('applicant_appl_overview C','B.appl_no = C.appl_no','left');
					$this->db->join('applicant_master D','C.reg_user_id = D.reg_user_id','left');
					$this->db->where("B.created_by",$logged_user);
					...
					if($txtRank[$i] != '')
					{
						...
						$this->db->where("program_code",$program_code);
						$this->db->where("category_code",$txtCategory);
						$this->db->select("cutoff_mark");
						$this->db->from("category_cutoff_details");
						...
						if($txtRank[$i] >= $cutoff_mark)
						{
							$cmbResult = 'Selected';
						}
						else
						{
							$cmbResult = 'Not Selected';
						}	
						if($cnt == '0')
						{
							
							//echo $cutoff_mark;die();
							$new_array = array( 
									"program_code"  	=>$program_code,
									"applicant_id"  	=>$txtRollNo,
									"applicant_name"  	=>$txtName,
			...
			case 'ADD_NEW_FILE':
				...
				$uploaddir1 = DOCUMENT_UPLOAD_URL.'/folder_list/';
				if(!is_dir($uploaddir))
					mkdir($uploaddir,0777,true);
			...
			case 'get_profile':
				...
				$this->db->select("full_name, am.reg_user_id, applicant_email, DATE_FORMAT(dob, '%d-%m-%Y') AS dob,program_name");
				$this->db->from('applicant_appl_overview AS aao');
				$this->db->join('applicant_master AS am', 'aao.reg_user_id = am.reg_user_id AND am.applied_program = aao.applied_program', 'LEFT');
				$this->db->join('program_master AS pm', 'pm.program_code = aao.applied_program', 'LEFT');
				if($program != '0')
				{
			...
			case 'get_InvigilatorData':
				$this->db->select("A.program_code,program_name,code,name,designation,from,mobile_no,mail_id");
				$this->db->from('invigilator_setup A');
				$this->db->join('program_master B','A.program_code = B.program_code','LEFT');
				$result = $this->db->get();
				$this->db->save_queries = TRUE;
				//echo $this->db->last_query();
				...
				foreach ($output_data as $aRow) 
	            {
	           	...
	           	return $output;
			break;
			case 'invigilator_code':
				$output = array("aaData" => array());
				$this->db->select("SUBSTRING(IFNULL(MAX(CODE), '0'), -3)+1  AS invigilator_code");
				$this->db->from("invigilator_setup");
				$result = $this->db->get();
				//echo $this->db->last_query();die();
				$output_data = $result->result_array();
				$output = array("aaData" => array());
				...
				foreach ($output_data as $aRow) 
                {
                    $invigilator_code = $aRow['invigilator_code'];
                }
				$new_sl_no = $invigilator_code;
				$changed_sl_no = '';
				if($new_sl_no < 10)
					$changed_sl_no = '00'.$new_sl_no;
				else if($new_sl_no < 100)
				...
				$invigilator_code = 'INV'.$changed_sl_no;
				
				$output = array('invigilator_code'=>$invigilator_code);
               	return $output;
			break;
			
			case 'Add_Invigilator_setup' :
				$institute_code = $this->session->userdata('institute_code');
				$logged_user = $this->session->userdata('user_name');
				$user = $logged_user.'_'.$institute_code;
				date_default_timezone_set('Asia/Kolkata');
				...
				$cmbProgram = isset($_POST['cmbProgram'])?$_POST['cmbProgram']:'';
				$txtInvigilatorCode = isset($_POST['txtInvigilatorCode'])?$_POST['txtInvigilatorCode']:'';
				$txtInvigilatorName = isset($_POST['txtInvigilatorName'])?$_POST['txtInvigilatorName']:'';
				$txtMailId = isset($_POST['txtMailId'])?$_POST['txtMailId']:'';
				$txtPhoneNo = isset($_POST['txtPhoneNo'])?$_POST['txtPhoneNo']:'';
				$txtFrom = isset($_POST['txtFrom'])?$_POST['txtFrom']:'';
				$txtDesignation = isset($_POST['txtDesignation'])?$_POST['txtDesignation']:'';
				
				$dbStatus = TRUE;
				$dbMessage = "Data Successfully Inserted";
				
				$new_data = array(
					'code'									=>$txtInvigilatorCode, 
					'program_code'							=>$cmbProgram,
					'name'									=>$txtInvigilatorName,
					'mail_id'								=>$txtMailId,
					'mobile_no'								=>$txtPhoneNo,
					'designation'							=>$txtDesignation,
					'from'									=>$txtFrom,
					'created_by'							=>$user,
					'created_on'							=>$date
				);
				$insert_user =  $this->db->insert('invigilator_setup', $new_data);
				if( ! $insert_user){
						$dbStatus = "ERROR";
						$dbMessage = "Error Inserting";
						$dbError = mysqli_error($con);	
			...
			case 'Update_Invigilator_setup' :
				...
				$this->db->where("program_code",$cmbProgram);
				$this->db->where("code",$txtInvigilatorCode);
				$sql = $this->db->update('invigilator_setup', $update_data);
				if( ! $sql){
						$dbStatus = "ERROR";
						$dbMessage = "Error Updating";
						$dbError = mysqli_error($con);	
			...
			case 'operation_delete_Invigilator_setup' :
				
				$dbStatus = TRUE;
				$dbMessage = "Data Successfully Deleted";
				$txtInvigilatorCode = isset($_POST['code'])?$_POST['code']:'';
				$cmbProgram = isset($_POST['post_code'])?$_POST['post_code']:'';
				$date = date('Y-m-d H:i:s', now());
				
				$this->db->where("program_code",$cmbProgram);
				$this->db->where("code",$txtInvigilatorCode);
				$delete_invigilator_setup = $this->db->delete('invigilator_setup');
				//echo $this->db->last_query();
				if(!$delete_invigilator_setup)
				{
					$dbstatus = FALSE;
					$dbmessage = 'Error deleting';
				}
