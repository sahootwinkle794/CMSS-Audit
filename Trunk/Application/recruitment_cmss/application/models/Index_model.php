        ...
        switch ($op) {
			...
			*/
			...
			case 'get_institutes':
				...
				$this->db->from('institute_master as ins');
				$this->db->where('ins.record_status',1);
				$this->db->where('ins.institute_code','RECINS001');
				$this->db->order_by('ins.institute_id','asc');
				//print_r($query);
				$result = $this->db->get();
				//echo $this->db->last_query(); die();
			...
			// Date:27-10-2021
			// This is used for fetch the program_master data
			case 'get_program_with_rank_data':
			    $queryset = "SET sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''))";
				$this->db->query($queryset);
				
				date_default_timezone_set('Asia/Kolkata');
        		$date = date('Y-m-d', now());
        		
				$this->db->select('A.program_group,A.program_name,A.program_code,A.program_start_date,
				A.program_end_date,A.apply_start_date,A.apply_end_date, action1.start_date AS action1_post_start_date, action1.end_date AS action1_post_end_date, A.advt_no,C.advt_path, A.program_desc,
				GROUP_CONCAT(DISTINCT P.corrigendum_name  ORDER BY P.id SEPARATOR "@") AS corrigendum_name,
				GROUP_CONCAT(DISTINCT P.corrigendum_path  ORDER BY P.id ASC SEPARATOR "@") AS corrigendum_path,
				GROUP_CONCAT( P.corrigendum_type  ORDER BY P.id ASC SEPARATOR "@") AS corrigendum_type');
				$this->db->from('program_master A');
				$this->db->join('advertisement_master C','A.program_group=C.program_group','LEFT');
				$this->db->join('corrigendum_upload P','A.program_code=P.program_code','LEFT');
				$this->db->join('(SELECT * FROM action_master WHERE action_name="Post Date" and record_status=1) AS action1',"action1.Program_code = A.program_code",'LEFT');
				$this->db->where('A.institute_code',$data);
				$this->db->where('A.record_status',1);
				$this->db->where('A.publish_status','YES');
				$this->db->group_by('A.program_code');
				...
				// return $result->result_array();
				$program_data = $result->result_array();

				// foreach ($program_data as &$item) {
				// 	$file = FCPATH . str_replace(base_url(), '', $item['advt_path']);
				// 	$item['advt_size'] = (file_exists($file) && is_file($file)) ? filesize($file) : 0;
				// }

				foreach ($program_data as &$item) {
					$advtPath = $item['advt_path'];
					$relativePath = '';

					// If advt_path contains base_url, strip it
					if (strpos($advtPath, base_url()) === 0) {
						$relativePath = str_replace(base_url(), '', $advtPath);
					}
					// If it contains your domain URL, strip that
					elseif (strpos($advtPath, 'https://gov.silicontechlab.com/cmss_new/recruitment_cmss/') === 0) {
						$relativePath = str_replace('https://gov.silicontechlab.com/cmss_new/recruitment_cmss/', '', $advtPath);
					}
					// Fallback: try to guess relative path
					elseif (strpos($advtPath, '/upload/') !== false) {
						$relativePath = substr($advtPath, strpos($advtPath, '/upload/'));
					}

					// Build full local path
					$file = FCPATH . ltrim($relativePath, '/\\');
			...
			case 'get_notice_data':
				date_default_timezone_set('Asia/Kolkata');
        		$date = date('Y-m-d', now());
				$this->db->select('A.corrigendum_name,A.corrigendum_path');
				$this->db->from('corrigendum_upload A');
				$this->db->join('program_master C','A.program_code=C.program_code','LEFT');
				$this->db->where('A.record_status',1);
				$this->db->where('A.corrigendum_type','NOTICE');
				$this->db->group_by('A.program_code');
			...
			case 'get_corrigendum_data':
				...
				$this->db->join('program_master C','A.program_code=C.program_code','LEFT');
				$this->db->where('A.record_status',1);
				$this->db->where('A.corrigendum_type','CORRIGENDUM');
				$this->db->group_by('A.program_code');
			...
			// Name :Jyotirmayee Khatua
			// Date:27-10-2021
			// This is used for fetch the advertisement_master data
			...
			// case 'get_post_data':
			// 	$sql="SELECT advertisement_master.advt_no FROM advertisement_master INNER JOIN  program_master ON advertisement_master.advt_no=program_master.advt_no "; 
			// 	$query = $this->db->query($sql);
			// 	// echo $this->db->last_query(); die();
			// 	// print_r($result);
			// 	return $result->result_array();
