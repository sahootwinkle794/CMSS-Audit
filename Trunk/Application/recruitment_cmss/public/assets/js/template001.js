...
var appl_status = $("#hidApplStatus").val();
	...
	$('#frmApplicantDetails').bootstrapValidator({
		...
		submitHandler: function(validator, form, submitButton) 
		{
				...
				$.ajax({
					url:base_url+"ajax_controller/course_wise_date_eligibility_001",
					type:"post",
					data:institutedata,
					success:function(response){  
						var res1 = JSON.parse(response);
						...
						else
						{
							...
							//ajax call to server
							$.ajax({
									url:base_url+"ajax_controller/save_applicant_details_temp_001",
									type:"post",
									data:formData,
									cache: false,
							        contentType: false,
	...
	$('#frmAcademicInfo').bootstrapValidator({
		...
		submitHandler: function(validator, form, submitButton) 
		{
			...
			//ajax call to server
			$.ajax({
				url:base_url+"ajax_controller/save_applicant_academic_details_temp_001",
				type:"post",
				data:formData,
				cache: false,
		        contentType: false,
        ...
        fields: {
			...
			txtMS1: {
                validators: {
					...
					callback: {
                        message: 'Total mark must be </br> greater than mark secured',
                        callback: function (value, validator, $field) {
                            if(parseInt(document.getElementById("txtFM1").value) < parseInt(value) )
							{
								return false;
							}else{
								return true;
			...
			txtMS2: {
                validators: {
					...
					callback: {
                        message: 'Total mark must be </br> greater than mark secured',
                        callback: function (value, validator, $field) {
                            if(parseInt(document.getElementById("txtFM2").value) < parseInt(value) )
							{
								return false;
							}else{
								return true;
			...
			txtMS3: {
                validators: {
					...
					callback: {
                        message: 'Total mark must be </br> greater than mark secured',
                        callback: function (value, validator, $field) {
                            if(parseInt(document.getElementById("txtFM3").value) < parseInt(value) )
							{
								return false;
							}else{
								return true;
	...
	$('#frmDeclaration').bootstrapValidator({
		...
		submitHandler: function(validator, form, submitButton) 
		{
			var formData = $('#frmApplicantDetails, #frmAcademicInfo, #frmDeclaration').serialize();
			//ajax call to server
			$.ajax({
				url:base_url+"ajax_controller/add_application_data_001",
				type:"post",
				data:formData,
				success:function(response)
				{  
   ...
   /* $("#radioYesMarks").click(function () {
    ...
    if($("input[name=radioComputerEducation]:checked").val()=="Other")
	{
		$("#divComp").show();	
	}
	else
	...
	$("#radioComputerEducationO").click(function () {
        $("#divComp").hide();
    });
    $("#radioComputerOth").click(function () {
        $("#divComp").show();
    });
