$(document).ready(function(){
	...
	function upload_result(){
		var programAdditional = $('#tbluploadresult').dataTable({
			"ajax":
			{
				"url": base_url+"/ajax_controller/get_general_info_data",
				"type": "POST",
				"data": ''
			},
			"bPaginate": true,
	...
	$('#frmAdditionalData').bootstrapValidator({
		...
		submitHandler: function(validator, form, submitButton) 
		{
			...
			{
				var action = 'update_general_info_data';
			}
			else
			{
				var action = 'insert_general_info_data';
			}
...
function deleteRowAdditional(event)
{
 ...
 function delete_resultdata(){
		...
		//ajax call to server
		$.ajax({
			url:base_url+"ajax_controller/delete_general_info",
			type:"post",
			data:institutedata,
			success:function(response)
			{  
