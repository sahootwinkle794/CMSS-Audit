$(document).ready(function(){
	...
	$("#btnUpdateMultiple").click(function()
    {
		...
		else
		{
		    ...
		    if(arr_cnt == 0)
		    {
				toastr.error("Please Select atleast one sms communication");
				return false;
			}
			else
			{
	...
	/*$("div.addbuttonCode").html('<button type="button" id="btnAssignEmail" class="btn btn-success"><i class="fa fa-check-square-o" aria-hidden="true"></i> Assign</button>&nbsp;</div>');*/
	...
	$("#btnAssignEmail").click(function()
    {
		...
		else
		{
		    ...
		    $('input[name="chkStatusEmail[]"]').each(function(){
		     ...
		     if(arr_cnt == 0)
		    {
				toastr.error("Please Select atleast one Email communication");
				return false;
			}
			else{
...
function getEmailCode()
{
	...
	$("[name='chkStatusEmail[]']").change(function () {
        if($('input[name="chkStatusEmail[]"][type=checkbox]').length == 0)
		{
			 $('#chkEmailAll').prop('checked', false);
		}
		else
...
{
    if($('input[name="chkFieldStatus[]"][type=checkbox]').length == 0)
	{
		 $('#chkEmailUpdate').prop('checked', false);
	}
	else
