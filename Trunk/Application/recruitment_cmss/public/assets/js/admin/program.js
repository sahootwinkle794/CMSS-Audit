$(document).ready(function(){
	...
	var programMaster = $('#tblProgramMaster').dataTable({
		...
		"aoColumns": [
						...
						{ "sName": "advt_no","bVisible":false },
						{ "sName": "advt_date","bVisible":false }
...
/*****AJAX CALL TO GET DATA FROM DATABASE(FOR SELECT OPTIONS)**********/
	...
	//get advertisement from advertisment master
	
	$('#cmbProgramGroup').change(function(){
		var grp = $('#cmbProgramGroup').val();
		$.ajax({
		url:base_url+"ajax_controller/get_advertisement",
		type:"post",
		data:{
			cmbProgramGroup:grp
		},
		success:function(response){  
			...
			var res1 = JSON.parse(response);
			$.each(res1.aaData,function(i,data){
				options = options + "<option a="+data.advtn_seq_no+" value='"+data.advt_no+"'>"+data.advt_no+"</option>";
				
			});
			$('#cmbadvertisement').html("");   //campusid from academicPeriod
			$('#cmbadvertisement').append(options);
			
			/*$('#cmbadvertisementedit').html(""); 
			$('#cmbadvertisementedit').append(options);*/
	...
	$('#cmbadvertisement').change(function(){
		var grp = $('#cmbadvertisement').val();
		$.ajax({
		url:base_url+"ajax_controller/get_advertisement_date",
		type:"post",
		data:{
			cmbadvertisement:grp
		},
		success:function(response){  
			var options = "<option value=''>Select</option>";
			//alert(response);
			var res1 = JSON.parse(response);
			console.log(res1);
			$('#txtAdvtDate').val(res1[0].advt_date);   //campusid from academicPeriod
			$('#txtAdvtDate').attr('disabled',true);
...
//*** *********** CHANGE BY ME*************************
		...
		submitHandler: function(validator, form, submitButton) 
		{
		   ...
		   /*var txtAgeStartdate = $('#txtAgeStartdate').val();
			...
			var institutedata={
				...
				cmbadvertisement:$("#cmbadvertisement").val(),
				cmbadvertisementseqno:$('#cmbadvertisement option:selected').attr('a')
			};
	...
	$('#txtStartdate').datepicker({
	    ...
	    //$('#txtAdmitCardStartdate').datepicker('setStartDate', startDate);
	    
	}).on('clearDate', function (selected) {
		$('#frmAddProgram').data('bootstrapValidator').updateStatus('txtStartdate', 'NOT_VALIDATED',null).validateField('txtStartdate');
	    $('#txtEnddate').datepicker('setStartDate', null);
	   // $('#txtAdmitCardStartdate').datepicker('setStartDate', null);
	...
	/*$('#txtAdmitCardEnddateCopy').datepicker({
	    ...
	    var startDate = new Date(selected.date.valueOf());
	    $('#txtAdmitCardEnddateCopy').datepicker('setStartDate', startDate);
	   	$('#txtAdmitCardStartdateCopy').datepicker().datepicker('setEndDate', startDate);
	...
	/*$('#txtAdmitCardEnddate').datepicker({
	    ...
	    var startDate = new Date(selected.date.valueOf());
	    $('#txtAdmitCardEnddate').datepicker('setStartDate', startDate);
	   	$('#txtAdmitCardStartdate').datepicker('setEndDate', startDate);
	...
	$('#txtAgeStartdate').datepicker({
	    ...
	    endDate: '+0d'
        	...
        	date2.setDate(date2.getDate() + 1);
			$("#txtAgeEnddate").datepicker().datepicker("setStartDate", date2);
		}).on('clearDate', function (selected) {
		//$('#frmAddProgram').data('bootstrapValidator').updateStatus('txtAgeStartdate', 'NOT_VALIDATED').validateField('txtAgeStartdate');
	...
	$('#txtAgeStartdateCopy').datepicker({
	    ...
	    var startDate = new Date(selected.date.valueOf());
	    var date2 = $('#txtAgeStartdateCopy').datepicker('getDate');
        date2.setDate(date2.getDate() + 1);
	    $('#txtAgeEnddateCopy').datepicker('setStartDate', date2);
	}).on('clearDate', function (selected) {
		//$('#frmCopyProgram').data('bootstrapValidator').updateStatus('txtAgeStartdateCopy', 'NOT_VALIDATED').validateField('txtAgeStartdateCopy');
	...
	$('#txtAgeEnddateCopy').datepicker({
	    ...
	    var startDate = new Date(selected.date.valueOf());
	    var date2 = $('#txtAgeEnddateCopy').datepicker('getDate');
        date2.setDate(date2.getDate() - 1);
	   	$('#txtAgeStartdateCopy').datepicker().datepicker('setEndDate', date2);
	}).on('clearDate', function (selected) {
		//$('#frmCopyProgram').data('bootstrapValidator').updateStatus('txtAgeEnddateCopy', 'NOT_VALIDATED').validateField('txtAgeEnddateCopy');
	    $('#txtAgeEnddateCopy').datepicker('setStartDate', null);
	});
	...
	$('#txtAdmitCardStartdateEdit').datepicker({
		...
		var start_date = $("#txtEnddateEdit").val();
		var end_date = $("#txtAdmitCardStartdateEdit").val();
		//alert(start_date);
		...
		{
			$("#txtAdmitCardStartdateEdit").val("");
			toastr.error("Admit Card Available Start Date can't be less than Application End Date");
			$('#frmProgramEdit').data('bootstrapValidator').updateStatus('txtAdmitCardStartdateEdit', 'NOT_VALIDATED').validateField('txtAdmitCardStartdateEdit');
		}
	}).on('clearDate', function (selected) {
		$('#frmProgramEdit').data('bootstrapValidator').updateStatus('txtAdmitCardStartdateEdit', 'NOT_VALIDATED').validateField('txtAdmitCardStartdateEdit');
	...
	function check_date(date1,date2)
	{
		...
		if(date2 == null || date2 == '')
			date2 = '00-00-0000';
		date_arr1 = date1.split("-");
		date_arr2 = date2.split("-");
		//alert(date_arr1[0]);
		//alert(date_arr2[0]);
		/*if(parseInt(date_arr1[2]) <= parseInt(date_arr2[2]))
		{
		...
		if(parseInt(date_arr1[1]) <= parseInt(date_arr2[1]))
		{
		...
		if(parseInt(date_arr1[0]) <= parseInt(date_arr2[0]))
		{
	...
	$('#txtEnddateEdit').datepicker({
		...
		var start_date = $("#txtStartdateEdit").val();
		var end_date = $("#txtEnddateEdit").val();
		//alert(start_date);
		if(check_date(start_date,end_date))
		{
		    ...
		    var startDate = new Date(selected.date.valueOf());
		    //$('#txtEnddateEdit').datepicker('setStartDate', startDate);
		   	$('#txtStartdateEdit').datepicker().datepicker('setEndDate', startDate);
		}
		else
		{
			$("#txtEnddateEdit").val("");
			toastr.error("Application End Date can't be less than Application Start Date");
			$('#frmProgramEdit').data('bootstrapValidator').updateStatus('txtEnddateEdit', 'NOT_VALIDATED').validateField('txtEnddateEdit');
		}
	...
	$('#txtAdmitCardEnddateEdit').datepicker({
		...
		var start_date = $("#txtAdmitCardStartdateEdit").val();
		var end_date = $("#txtAdmitCardEnddateEdit").val();
		//alert(start_date);
		if(check_date(start_date,end_date))
		{
		    ...
		    var startDate = new Date(selected.date.valueOf());
		   	$('#txtAdmitCardStartdateEdit').datepicker().datepicker('setEndDate', startDate);
		}
		else
		{
			$("#txtAdmitCardEnddateEdit").val("");
			toastr.error("Admit Card Available End Date can't be less than Admit Card Available Start Date");
			$('#frmProgramEdit').data('bootstrapValidator').updateStatus('txtAdmitCardEnddateEdit', 'NOT_VALIDATED').validateField('txtAdmitCardEnddateEdit');
		}
	...
	$('#txtAgeStartdateEdit').datepicker({
        ...
        date2.setDate(date2.getDate() + 1);
	    $('#txtAgeEnddateEdit').datepicker('setStartDate', date2);
	}).on('clearDate', function (selected) {
		//$('#frmProgramEdit').data('bootstrapValidator').updateStatus('txtAgeStartdateEdit', 'NOT_VALIDATED').validateField('txtAgeStartdateEdit');
	...
	$('#txtAgeEnddateEdit').datepicker({
        ...
        date2.setDate(date2.getDate() - 1);
	   	$('#txtAgeStartdateEdit').datepicker().datepicker('setEndDate', date2);
	}).on('clearDate', function (selected) {
		//$('#frmProgramEdit').data('bootstrapValidator').updateStatus('txtAgeEnddateEdit', 'NOT_VALIDATED').validateField('txtAgeEnddateEdit');
	    $('#txtAgeEnddateEdit').datepicker('setStartDate', null);
	});
	...
	/*$('#txtAppStartdate').datetimepicker({
	    ...
	    var startDate = new Date(selected.date.valueOf());
	    $('#txtAppEnddate').datetimepicker('setStartDate', startDate);
	}).on('clearDate', function (selected) {
		$('#frmAddProgram').data('bootstrapValidator').updateStatus('txtAppStartdate', 'NOT_VALIDATED').validateField('txtAppStartdate');
	    $('#txtAppEnddate').datetimepicker('setStartDate', null);
	});*/
	/*$('#txtAppStartdate').datetimepicker({ 
			dateFormat: 'dd-mm-yy',
	...
	$('#txtAppEnddate').datetimepicker({
		dateFormat: 'dd-mm-yy',
		onSelect: function (date) {
        var date2 = $('#txtAppEnddate').datetimepicker('getDate');
        date2.setDate(date2.getDate() + 1);
	...
	/*$('#txtAppStartdateEdit').datetimepicker({ 
        ...
        $('#frmAddProgram').data('bootstrapValidator').updateStatus('txtAppStartdateEdit', 'VALID', null);
		//$('#txtAppStartdate').datetimepicker('option', 'minDate', date2); 
    	}
	...
	$('#txtAppEnddateEdit').datetimepicker({
		dateFormat: 'dd-mm-yy',
		onSelect: function (date) {
        var date2 = $('#txtAppEnddateEdit').datetimepicker('getDate');
        date2.setDate(date2.getDate() + 1);
        $('#txtAppEnddateEdit').datetimepicker('option', 'maxDate', date2);
        $('#frmAddProgram').data('bootstrapValidator').updateStatus('txtAppEnddateEdit', 'VALID', null);
		//$('#txtAppStartdate').datetimepicker('option', 'minDate', date2); 
    	}
	...
	$('#txtAdmitCardStartdate').datepicker({
	    ...
	    var date2 = $('#txtAdmitCardStartdate').datepicker('getDate');
        date2.setDate(date2.getDate() + 1);
	   	$('#txtAdmitCardEnddate').datepicker('setStartDate', date2);
	});
	...
	$('#txtAdmitCardEnddate').datepicker({
	    ...
	    var date2 = $('#txtAdmitCardEnddate').datepicker('getDate');
	    console.log('date2',date2);
       if(date2 == '' || date2 == 'Invalid Date')
       {
       		toastr.error('Please Select Appliction Start Date');
	   		$('#txtAdmitCardEnddate').val('');
	   }
	});
	...
	$('#txtAppStartdate').datepicker({
	    ...
	    var date2 = $('#txtAppStartdate').datepicker('getDate');
        date2.setDate(date2.getDate() + 1);
	   	$('#txtAppEnddate').datepicker('setStartDate', date2);
	});
	...
	$('#txtAppStartdateEdit').datepicker({
	    ...
	    var date2 = $('#txtAppStartdateEdit').datepicker('getDate');
        date2.setDate(date2.getDate() + 1);
	   	$('#txtAppEnddateEdit').datepicker('setStartDate', date2);
	});
	...
	/*$('#txtAppEnddate').datetimepicker({
	    ...
	    var startDate = new Date(selected.date.valueOf());
	    $('#txtAppEnddate').datetimepicker('setStartDate', startDate);
	    $('#txtAppStartdate').datetimepicker('setEndDate', startDate);
	}).on('clearDate', function (selected) {
		$('#frmAddProgram').data('bootstrapValidator').updateStatus('txtAppEnddate', 'NOT_VALIDATED').validateField('txtAppEnddate');
	    $('#txtAppEnddate').datetimepicker('setStartDate', null);
	});
	...
	$('#txtAppEnddateEdit').datepicker({
		...
		var start_date = $("#txtAppStartdateEdit").val();
		var end_date = $("#txtAppEnddateEdit").val();
		//alert(start_date);
		if(check_date(start_date,end_date))
		{
	    	...
	    	var startDate = new Date(selected.date.valueOf());
	    //$('#txtAppEnddateEdit').datepicker('setStartDate', startDate);
	   		$('#txtAppStartdateEdit').datepicker().datepicker('setEndDate', startDate);
		}
		else
		{
			$("#txtAppEnddateEdit").val("");
			toastr.error("Program End Date can't be less than Program Start Date");
			$('#frmProgramEdit').data('bootstrapValidator').updateStatus('txtAppEnddateEdit', 'NOT_VALIDATED').validateField('txtEnddateEdit');
		}
	...
	$('#txtEnddateCopy').datepicker({
	    ...
	    var startDate = new Date(selected.date.valueOf());
	    $('#txtEnddateCopy').datepicker('setStartDate', startDate);
	   	$('#txtStartdateCopy').datepicker().datepicker('setEndDate', startDate);
	}).on('clearDate', function (selected) {
		$('#frmCopyProgram').data('bootstrapValidator').updateStatus('txtEnddateCopy', 'NOT_VALIDATED').validateField('txtEnddateCopy');
	    $('#txtEnddateCopy').datepicker('setStartDate', null);
	});
	...
	$('#txtAppEnddateCopy').datepicker({
	    ...
	    var startDate = new Date(selected.date.valueOf());
	    $('#txtAppEnddateCopy').datepicker('setStartDate', startDate);
	   	$('#txtAppStartdateCopy').datepicker().datepicker('setEndDate', startDate);
	}).on('clearDate', function (selected) {
		$('#frmCopyProgram').data('bootstrapValidator').updateStatus('txtAppEnddateCopy', 'NOT_VALIDATED').validateField('txtAppEnddateCopy');
	    $('#txtAppEnddateCopy').datepicker('setStartDate', null);
	});
...
function editold(event)
{
	...
	//alert(oTable.fnGetData(row)['omr_no']);
		...
		{  
			...
			{	
				var dtblProgram = $("#tblProgramMaster").DataTable();
	 			...
	 			dtblProgram.ajax.reload();
	 			var tblProgramMasterOld = $("#tblProgramMasterOld").DataTable();
 				tblProgramMasterOld.ajax.reload();
				toastr.success(result.msg);		
			}
			else
			{
...
function get_advt_data(cmbProgramGroupEdit,advt_no)
	{
		//alert(cmbProgramGroupEdit);
		var grp = cmbProgramGroupEdit;
		$.ajax({
		url:base_url+"ajax_controller/get_advertisement",
		type:"post",
		data:{
		...
		success:function(response){  
			...
			$.each(res1.aaData,function(i,data){
				...
				if(data.advt_no == advt_no)
		   		{
					options = options + "<option a="+data.advtn_seq_no+" value='"+data.advt_no+"' selected>"+data.advt_no+"</option>";
				}
				else
				{
					options = options + "<option a="+data.advtn_seq_no+" value='"+data.advt_no+"'>"+data.advt_no+"</option>";
...
function edit(event)
{
	...
	var admitcard_start_date = oTable.fnGetData(row)['admitcard_start_date'];
	var admitcard_end_date = oTable.fnGetData(row)['admitcard_end_date'];
	var cmbProgramGroupEdit = oTable.fnGetData(row)['program_group_code'];
	var advt_no = oTable.fnGetData(row)['advt_no'];
	//alert(advt_no);
	get_advt_data(cmbProgramGroupEdit,advt_no);
	//alert(sequence_no);
	var birthstartdate = oTable.fnGetData(row)['birth_start_date'];
	var birthenddate = oTable.fnGetData(row)['birth_end_date'];
	$('#hidUniqueidEdit').val(programCode);//GETTING VALUE FOR HIDDEN COLUMN
	...
	$('#cmbProgramGroupEdit').val(oTable.fnGetData(row)['program_group']);
	$('#taElectiveSubjectsEdit').val(oTable.fnGetData(row)['elective_subjects']);
	$('#cmbadvertisementedit').val(oTable.fnGetData(row)['advt_no']);
	$('#txtAdvtDateEdit').val(oTable.fnGetData(row)['advt_date']);
	...
	$('#frmProgramEdit').bootstrapValidator({
		...
		submitHandler: function(validator, form, submitButton) 
		{
			...
			var institutedata={
				...
				txtOnlineTransactionNoEdit:$('#txtOnlineTransactionNoEdit').val(),
				txtOmrNoEdit:$('#txtOmrNoEdit').val(),
				txtAdvtNoEdit:$('#cmbadvertisementedit').val(),
				txtAdvtDateEdit:$('#txtAdvtDateEdit').val(),
				cmbStatusEdit:$('#cmbStatusEdit').val(),
				cmbRegistrationTemplateEdit:$('#cmbRegistrationTemplateEdit').val(),
				cmbTemplateEdit:$('#cmbTemplateEdit').val(),
				txtStartdateEdit:$('#txtStartdateEdit').val(),
