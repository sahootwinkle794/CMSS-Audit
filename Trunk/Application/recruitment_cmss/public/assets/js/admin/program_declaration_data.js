$(document).ready(function(){
	...
	var programAdditional = $('#tblprogramdeclaration').dataTable({
		"ajax":
		{
			"url": base_url+"/ajax_controller/get_program_declaration_data",
			"type": "POST",
			"data": ''
		},
		"bPaginate": true,
		...
		"aoColumns": [
						...
						{"sName": "Action","sWidth": "10%", "sClass":"dt-center", "sDefaultContent": "<button class='btn btn-warning btn-sm btn-circle tooltipTable' onclick='ProgramDeclarationSetup(event,\"edit\")' title='Edit' ><i class='fa fa-edit'></i></button>\
			            <button class='btn btn-danger btn-sm btn-circle tooltipTable' onclick='ProgramDeclarationSetup(event,\"delete\")' title='Delete' ><i class='fa fa-trash'></i>"}
        ],
	...
	{
		$('#programdeclarationForm').data('bootstrapValidator').resetForm(true); 
		$("#hidAction").val('Add');
		...
		$("#myModalLabel").html("Add Program Declaration"); 
	...
	$('#programdeclarationForm').bootstrapValidator({
		...
		{
			var formData = new FormData(document.getElementById("programdeclarationForm"));
			
			var oper = $("#hidAction").val();
			//ajax call to server
			if(oper == 'Add')
				oper = 'Add_programdeclaration_setup';
			else if(oper == 'Update')
				oper = 'Update_programdeclaration_setup'; 
			...
			$.ajax({
				...
				{  
					...
					if(res.status == true){
						var tblprogramdeclaration = $("#tblprogramdeclaration").DataTable();
			 			tblprogramdeclaration.ajax.reload();
			 			$('#programdeclarationForm').data('bootstrapValidator').resetForm(true);	
						toastr.success(res.msg);
						if(oper != 'Add_programdeclaration_setup')
						{
...
function ProgramDeclarationSetup(event,action)
{
	var oTable = $('#tblprogramdeclaration').dataTable();
	var row;
	...
	if(action == 'edit')
	{
		$("#myModalLabel").html("Update Program Declaration");
	...
	else
	{
	    ...
	    function deleteMaster(){ 
			...
			{
				hidid :$('#hidid').val(), 
				type:"delete_programdeclaration_Setup"
			};	
			type = 'delete_programdeclaration_Setup';	
			//ajax call to server
			$.ajax({
