...
$(document).ready(function(){
	...
	$.ajax({
		...
		{  
			...
			$('#cmbProgramGroupQual').html(options);
			$('#cmbProgramGroupdeclar').html("");   
			$('#cmbProgramGroupdeclar').html(options);
	...
	$('#cmbProgramGroupdeclar').change(function()
	{
		get_program_data_declar();
	});
	$.ajax({
		url:base_url+"ajax_controller/select_program_data_manage_app", 
		type:"post",
		...
		{  
			...
			// console.log(options);
			$('#cmbProgramGroupQual2').html("");   
			$('#cmbProgramGroupQual2').html(options);
	...
	// $('#cmbProgramGroupQual2').change(function()
	// {
		
	// 	//}
	// });
	/***********************************************start of degree course*******************************/
	var tbldeclarationMaster = $('#tbldeclarationMaster').dataTable({		
		//"sAjaxSource": base_url+"ajax_controller/get_religion",
		"ajax":
			{
				"url": base_url+"ajax_controller/get_declaration_data",
				"type": "POST"				
			},
		"bPaginate": true,
        "bLengthChange": true,
        ...
        "bInfo": true,
        "bAutoWidth": false,
		"sDom":"<'row'<'col-xs-4'i><'col-xs-4'l><'col-xs-4'f>r>t<'row'<'col-xs-6' <'row' <'col-xs-7 institutedeclarationbutton' >>><'col-xs-6'p>>",
		"aoColumns": [
	                     { "sName": "sl_no","sWidth": "5%" },
	                     { "sName": "program_group","bVisible":false },
	                     { "sName": "program_name","sWidth": "25%" },
	                     { "sName": "program_code","bVisible":false },
	                     { "sName": "declaration","sWidth": "20%" },
	                     { "sName": "id","bVisible":false },
	                     // { "sName": "id","sWidth": "10%" },
	                     { "sName": "Action","sClass": "alignCenter",data:null,"sWidth": "10%", "sDefaultContent": "<button class='btn btn-warning  tooltipTable btn-circle' title='Edit' id = 'btnEditDeclaration' onclick='DeclarationRow(event,\"edit\")'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></button>&nbsp;&nbsp;<button id='btnDeleteDeclaration' title='Delete' class='btn btn-danger tooltipTable btn-circle' onclick='DeclarationRow(event,\"delete\")'><i class='fa fa-trash-o' aria-hidden='true'></i></button>"}
	...
	/**CREATING BUTTON*****/
	$("div.institutedeclarationbutton").html('<button id="btnAddDeclaration" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i> Add</button>&nbsp;</div>');
	
	$('#btnAddDeclaration').click(function(e){
		//$('#frmDegree').data('bootstrapValidator').resetForm(true);//Reseting the tick marks before opening add modal	
		
		$(tbldeclarationMaster.fnSettings().aoData).each(function ()
		{
			$(this.nTr).removeClass('success');
		});
		$('#cmbProgramGroupdeclar').val("");
		$('#cmbSelectProgramdeclar').val("");
		$('#txtDeclaration').val("");
		$('#hidDeclarationid').val("");
		$('#hidOperDeclaration').val("add_declaration");
		$('#DeclarationModal').modal('show');
		
		$('#DeclarationModal').on('shown.bs.modal', function () 
		{ 
			$('#txtDeclaration').focus(); // Focusing the textbox
		})
	});
	...
	$('#frmDeclaration').bootstrapValidator({
		...
		submitHandler: function(validator, form, submitButton) 
		{
			...
			var formData = new FormData(document.getElementById("frmDeclaration"));
			// var program_code = serealizeSelects($('.cmbSelectProgramCourse'));
			// alert('234',$('#hidCourseeditid').val());
			if($('#hidDeclarationid').val() == '' || $('#hidDeclarationid').val() == 'undefined')
			{
				var caseajax = 'add_declaration';
			}
			else
			{
				var caseajax = 'edit_declaration';
				
			}
			//ajax call to server
			$.ajax({
				url:base_url+"ajax_controller/"+caseajax,
				type:"post",
				data: formData,
				cache: false,
		        contentType: false,
				...
				success:function(response){  
					...
					{*/
						...
						if(obj.status == true)
						{
							...
							var dtblProgram = $("#tbldeclarationMaster").DataTable();
				 			dtblProgram.ajax.reload();
				 			$('#frmDeclaration').data('bootstrapValidator').resetForm(true);
		...
		fields: {
					cmbProgramGroupQual2: {							//form input type name
						validators: {
							notEmpty: {
					...
					cmbSelectProgram2: {							//form input type name
						validators: {
							notEmpty: {
								message: 'This field can\'t left blank'
							}
					...
					cmbQualificationcourse: {							//form input type name
						validators: {
							notEmpty: {
								message: 'This field can\'t left blank'
							}
					...
					cmbdegree: {							//form input type name
						validators: {
							notEmpty: {
								message: 'This field can\'t left blank'
							}
	...
	/***********************************************start of degree course*******************************/
	var tblqualdegreecoursemapMaster = $('#tblqualdegreecoursemapMaster').dataTable({		
		//"sAjaxSource": base_url+"ajax_controller/get_religion",
		"ajax":
			{
				"url": base_url+"ajax_controller/get_degree_course_data",
				"type": "POST"				
			},
		"bPaginate": true,
        "bLengthChange": true,
        ...
        "bInfo": true,
        "bAutoWidth": false,
		"sDom":"<'row'<'col-xs-4'i><'col-xs-4'l><'col-xs-4'f>r>t<'row'<'col-xs-6' <'row' <'col-xs-7 institutecoursebutton' >>><'col-xs-6'p>>",
		"aoColumns": [
	                     { "sName": "sl_no","sWidth": "5%" },
	                     { "sName": "course_id","bVisible":false },
	                     { "sName": "qualification_code","bVisible":false },
	                     { "sName": "program_name","sWidth": "25%" },
	                     { "sName": "qualification_name","sWidth": "20%" },
	                     { "sName": "degree_name","sWidth": "20%" },
	                     { "sName": "course_name","sWidth": "20%" },
	                     { "sName": "program_group","bVisible":false },
	                     { "sName": "program_code","bVisible":false },
	                     // { "sName": "id","sWidth": "10%" },
	                     { "sName": "Action","sClass": "alignCenter",data:null,"sWidth": "10%", "sDefaultContent": "<button class='btn btn-warning  tooltipTable btn-circle' title='Edit' id = 'btnEditCourse' onclick='CourseRow(event,\"edit\")'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></button>&nbsp;&nbsp;<button id='btnDeletecourse' title='Delete' class='btn btn-danger tooltipTable btn-circle' onclick='CourseRow(event,\"delete\")'><i class='fa fa-trash-o' aria-hidden='true'></i></button>"}
	...
	/**CREATING BUTTON*****/
	$("div.institutecoursebutton").html('<button id="btnAddCourse" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i> Add</button>&nbsp;</div>');
	/*****END OF BUTTON CREATION******/
	
	$('#btnAddCourse').click(function(e){
		//$('#frmDegree').data('bootstrapValidator').resetForm(true);//Reseting the tick marks before opening add modal	
		
		$(tblqualdegreecoursemapMaster.fnSettings().aoData).each(function ()
		{
			$(this.nTr).removeClass('success');
		});
		$('#cmbProgramGroup2').val("");
		$('#cmbSelectProgram2').val("");
		$('#cmbQualification').val("");
		$('#cmbdegree').val("");
		$('#txtcourse').val("");
		$('#hidCourseeditid').val("");
		$('#hidOperCourse').val("add_course");
		$('#CourseAddModal').modal('show');
		
		$('#CourseAddModal').on('shown.bs.modal', function () 
		{ 
			$('#cmbQualification').focus(); // Focusing the textbox
		})
	});
	...
	$('#frmCourse').bootstrapValidator({
		...
		{
			$("#spanProcessingCountryAdd").show();
			var formData = new FormData(document.getElementById("frmCourse"));
			// var program_code = serealizeSelects($('.cmbSelectProgramCourse'));
			// alert('234',$('#hidCourseeditid').val());
			if($('#hidCourseeditid').val() == '' || $('#hidCourseeditid').val() == 'undefined')
			{
				var caseajax = 'add_degree_wise_course';
			}
			else
			{
				var caseajax = 'edit_degree_wise_course';
				...
				success:function(response){  
					...
					{*/
						...
						if(obj.status == true)
						{
							//$("#errorlog_country_add").html("");
							toastr.success(obj.msg);
							var dtblProgram = $("#tblqualdegreecoursemapMaster").DataTable();
				 			dtblProgram.ajax.reload();
				 			$('#frmCourse').data('bootstrapValidator').resetForm(true);
	...
	/***************************************Qualification Degree Tab****************************************/
	var tblqualdegreemapMaster = $('#tblqualdegreemapMaster').dataTable({		
		//"sAjaxSource": base_url+"ajax_controller/get_religion",
		"ajax":
			{
				"url": base_url+"ajax_controller/get_qual_degree_data",
				"type": "POST"				
			},
		"bPaginate": true,
        "bLengthChange": true,
        ...
        "bInfo": true,
        "bAutoWidth": false,
		"sDom":"<'row'<'col-xs-4'i><'col-xs-4'l><'col-xs-4'f>r>t<'row'<'col-xs-6' <'row' <'col-xs-7 institutedegreebutton' >>><'col-xs-6'p>>",
		"aoColumns": [
	                     { "sName": "sl_no","sWidth": "10%" },
	                     { "sName": "program_group","bVisible":false },
	                     { "sName": "program_name","sWidth": "25%" },
	                     { "sName": "program_code","bVisible":false  },
	                     { "sName": "qualification_name","sWidth": "25%" },
	                     { "sName": "qualification_code","bVisible":false  },
	                     { "sName": "degree_name","sWidth": "20%" },
	                     { "sName": "id","bVisible":false  },
	                     { "sName": "Action","sClass": "alignCenter",data:null,"sWidth": "20%", "sDefaultContent": "<button class='btn btn-warning  tooltipTable btn-circle' title='Edit' id = 'btnEditDegree' onclick='DegreeRow(event,\"edit\")'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></button>&nbsp;&nbsp;<button id='btnDeletedegree' title='Delete' class='btn btn-danger tooltipTable btn-circle' onclick='DegreeRow(event,\"delete\")'><i class='fa fa-trash-o' aria-hidden='true'></i></button>"}
	...
	/**CREATING BUTTON*****/
	$("div.institutedegreebutton").html('<button id="btnAddDegree" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i> Add</button>&nbsp;</div>');
	/*****END OF BUTTON CREATION******/
	
	$('#btnAddDegree').click(function(e){
		//$('#frmDegree').data('bootstrapValidator').resetForm(true);//Reseting the tick marks before opening add modal	
		$(tblqualdegreemapMaster.fnSettings().aoData).each(function ()
		{
			$(this.nTr).removeClass('success');
		});
		$('#cmbProgramGroupQual').val("");
		$('#cmbSelectProgram').val("");
		$('#cmbQualification').val("");
		$('#txtDegreeName').val("");
		$('#hidDegreeditid').val("");
		$('#hidOperDegree').val("add_degree");
		$('#DegreeAddModal').modal('show');
		
		$('#DegreeAddModal').on('shown.bs.modal', function () 
		{ 
			$('#cmbQualification').focus(); // Focusing the textbox
		})
	});
	
	$('#frmDegree').bootstrapValidator({
		message: 'This value is not valid',
        feedbackIcons: {
            valid: 'glyphicon glyphicon-ok',
            invalid: 'glyphicon glyphicon-remove',
		...
		{
			$("#spanProcessingCountryAdd").show();
			var formData = new FormData(document.getElementById("frmDegree"));
			if($('#hidDegreeditid').val() == '' || $('#hidDegreeditid').val() == 'undefined')
			{
				var caseajax = 'add_degree';
			}
			else
			{
				var caseajax = 'edit_degree';
				...
				success:function(response){  
					try
					{
						...
						if(obj.status == true)
						{
							//$("#errorlog_country_add").html("");
							toastr.success(obj.msg);
							var dtblProgram = $("#tblqualdegreemapMaster").DataTable();
				 			dtblProgram.ajax.reload();
				 			$('#frmDegree').data('bootstrapValidator').resetForm(true);
				 			$('#DegreeAddModal').modal('hide');	
							//toastr.success('Data Successfully Inserted');				
		...
		fields: {
					cmbQualification: {							//form input type name
						validators: {
							notEmpty: {
								message: 'This field can\'t left blank'
							}
	...
	var religionMaster = $('#tblreligionMaster').dataTable({		
		...
		"ajax":
			{
				"url": base_url+"ajax_controller/get_religion",
				"type": "POST"				
			},
		"bPaginate": true,
        "bLengthChange": true,
        ...
        "bInfo": true,
        "bAutoWidth": false,
		"sDom":"<'row'<'col-xs-4'i><'col-xs-4'l><'col-xs-4'f>r>t<'row'<'col-xs-6' <'row' <'col-xs-7 instituterelbutton' >>><'col-xs-6'p>>",
		"aoColumns": [
	                     { "sName": "sl_no","sWidth": "5%" },
	                     { "sName": "religion_code" },
	                     { "sName": "religion_name" },
	                     { "sName": "id","bVisible":false },
	                     { "sName": "Action","sClass": "alignCenter",data:null,"sWidth": "10%", "sDefaultContent": "<button class='btn btn-warning  tooltipTable btn-circle' title='Edit' id = 'btnEditReligion' onclick='ReligionRow(event,\"edit\")'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></button>&nbsp;&nbsp;<button id='btnDeleteReligion' title='Delete' class='btn btn-danger tooltipTable btn-circle' onclick='ReligionRow(event,\"delete\")'><i class='fa fa-trash-o' aria-hidden='true'></i></button>"}
	...
	/**CREATING BUTTON*****/
	$("div.instituterelbutton").html('<button id="btnAddReligion" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i> Add</button>&nbsp;</div>');
	/*****END OF BUTTON CREATION******/
	
	$('#btnAddReligion').click(function(e){
		
		$('#frmReligion').data('bootstrapValidator').resetForm(true);//Reseting the tick marks before opening add modal	
	...
	$('#frmReligion').bootstrapValidator({
		message: 'This value is not valid',
        feedbackIcons: {
            valid: 'glyphicon glyphicon-ok',
            invalid: 'glyphicon glyphicon-remove',
		...
		{
			$("#spanProcessingCountryAdd").show();
			var formData = new FormData(document.getElementById("frmReligion"));
			if($('#hidReligioneditid').val() == '' || $('#hidReligioneditid').val() == 'undefined')
			{
				var caseajax = 'add_religion';
			}
			else
			{
				var caseajax = 'edit_religion';
					...
					try
					{
						...
						if(obj.status == true)
						{
							...
							var dtblProgram = $("#tblreligionMaster").DataTable();
				 			dtblProgram.ajax.reload();
				 			$('#frmReligion').data('bootstrapValidator').resetForm(true);
...
function get_program_data_declar(program_code = '')
{
	
	var program_group = $("#cmbProgramGroupdeclar").val();
	var program_type = '';
		var institutedata = {
		...
		$.ajax({
			...
			{  
				...
				$('#cmbSelectProgramdeclar').html("");   
				$('#cmbSelectProgramdeclar').html(options);
				if(program_code != '')
				{
					$('#cmbSelectProgramdeclar').val(program_code);   
				}
...
function get_program_data(program_code = '')
{
	...
	var program_type = '';
		...
		$.ajax({
			...
			{  
				...
				$('#cmbSelectProgram').html("");   
				$('#cmbSelectProgram').html(options);
				if(program_code != '')
				{
					$('#cmbSelectProgram').val(program_code);   
				}
...
function ReligionRow(event,action)
{
	$('#frmReligion').data('bootstrapValidator').resetForm(true);
	...
	//alert(id);
	$('#hidReligioneditid').val(ID);
	$('#txtReligionCode').val(oTable.fnGetData(row)['religion_code']);
	$('#txtReligionName').val(oTable.fnGetData(row)['religion_name']);
	
	if(action == 'edit')
	{
		$('#hidOperReligion').val("edit_religion");	
		$('#ReligionAddModal').modal('show');
	}	
	else
	{
		...
		swal({
		  title: "Are you sure?",
		  text: "you want to Delete the Religion!",
		  type: "warning",
...
function DegreeRow(event,action)
{
	$('#frmDegree').data('bootstrapValidator').resetForm(true);
	var oTable = $('#tblqualdegreemapMaster').dataTable();		
	$(oTable.fnSettings().aoData).each(function ()
	{
		$(this.nTr).removeClass('success');
	});			
	...
	//alert(id);
	$('#hidDegreeditid').val(ID);
	$('#cmbProgramGroupQual').val(oTable.fnGetData(row)['program_group']);
	get_program_data(program_code);
	$('#cmbQualification').val(oTable.fnGetData(row)['qualification_code']);
	$('#txtDegreeName').val(oTable.fnGetData(row)['degree_name']);
	
	if(action == 'edit')
	{
		$('#hidOperDegree').val("edit_degree");	
		$('#DegreeAddModal').modal('show');
	}	
	else
	{
		...
		function(isConfirm){
		  if (isConfirm) {
		  	deleteCode(ID);
		    swal("Deleted", "Degree has been deleted successfully.", "success");
		  } else {
			    swal("Cancelled", "Degree is safe ", "error");
		  }
		});
		function deleteCode(ID)
		{
			$.ajax({
				url:base_url+"ajax_controller/delete_degree",
				type:"post",
				data:{
					'ID' : ID
				},
...
function CourseRow(event,action)
{
	$('#frmCourse').data('bootstrapValidator').resetForm(true);
	var oTable = $('#tblqualdegreecoursemapMaster').dataTable();		
	$(oTable.fnSettings().aoData).each(function ()
	{
		$(this.nTr).removeClass('success');
	});			
    ...
    var ID = oTable.fnGetData(row)['course_id'];
	//alert(id);
	$('#hidCourseeditid').val(ID);
	$('#cmbProgramGroupQual2').val(oTable.fnGetData(row)['program_group']);
	change_cmbProgramGroupQual2(oTable.fnGetData(row)['program_code']);
	$('#cmbQualificationcourse').val(oTable.fnGetData(row)['qualification_code']);
	change_cmbQualificationcourse(oTable.fnGetData(row)['degree_name']);
	$('#txtcourse').val(oTable.fnGetData(row)['course_name']);
	
	if(action == 'edit')
	{
		$('#hidOperCourse').val("edit_course");	
		$('#CourseAddModal').modal('show');
	}	
	else
	{	
		...
		{
			$.ajax({
				url:base_url+"ajax_controller/delete_course",
				type:"post",
				data:{
					'ID' : ID
				},
				...
				{  
					...
					{*/
						...
						{
							sweetAlert("Course",obj.msg, "error");	
						}
					/*}
					catch(e)
					{
...
function DeclarationRow(event,action)
{
	$('#frmDeclaration').data('bootstrapValidator').resetForm(true);
	var oTable = $('#tbldeclarationMaster').dataTable();		
	$(oTable.fnSettings().aoData).each(function ()
	{
		$(this.nTr).removeClass('success');
	});			
    ...
    var ID = oTable.fnGetData(row)['id'];
	//alert(id);
	$('#hidDeclarationid').val(ID);
	$('#cmbProgramGroupdeclar').val(oTable.fnGetData(row)['program_group']);
	$('#txtDeclaration').val(oTable.fnGetData(row)['declaration']);
	//$('#txtDeclaration').html(oTable.fnGetData(row)['declaration']);
	get_program_data_declar(oTable.fnGetData(row)['program_code']);
	
	
	if(action == 'edit')
	{
		$('#hidOperDeclaration').val("edit_declaration");	
		$('#DeclarationModal').modal('show');
	}	
	else
	{	
		...
		function(isConfirm){
		  if (isConfirm) {
		  	deleteCode(ID);
		    swal("Deleted", "Declaration has been deleted successfully.", "success");
		  } else {
			    swal("Cancelled", "Degree is safe ", "error");
		  }
		});
		...
		{
			$.ajax({
				url:base_url+"ajax_controller/delete_declaration",
				type:"post",
				data:{
					'ID' : ID
				},
...
// $('#cmbQualificationcourse').change(function()
function change_cmbQualificationcourse(did = null)
{
	var program = $('#cmbSelectProgram2').val();
	var cmbQualificationcourse = $("#cmbQualificationcourse").val();
	var institutedata = {
		cmbQualificationcourse:cmbQualificationcourse,
		program:program,
	};
	$.ajax({
		url:base_url+"/ajax_controller/select_qual_wise_degree",
		type:"post",
		data:institutedata,
		success:function(response)
		{  
			...
			$.each(res1.aaData,function(i,data)
			{
				if(did != null && did == data.degree_name){
					options = options + "<option value='"+data.degree_name+"' selected>"+data.degree_name+"</option>";
				}else{
					options = options + "<option value='"+data.degree_name+"'>"+data.degree_name+"</option>";
				}
			});
			// console.log(options);
			$('#cmbdegree').html("");   
			$('#cmbdegree').html(options);
		},
		error:function()
		{
			alert("We are unable to Process.Please contact Support");
...
function change_cmbProgramGroupQual2(sid = null) {
	var selectedOption = '';
	//
	var program_group = $("#cmbProgramGroupQual2").val();
	var institutedata = {
		program_group:program_group,
	};
	$.ajax({
		...
		{  
			...
			$.each(res1.aaData,function(i,data)
			{
				if(sid != null && sid == data.program_code){
					options = options + "<option value='"+data.program_code+"' selected>"+data.program_name+"</option>";
				}else{
					options = options + "<option value='"+data.program_code+"'>"+data.program_name+"</option>";
				}
			});
			$('#cmbSelectProgram2').html("");   
			$('#cmbSelectProgram2').html(options);
