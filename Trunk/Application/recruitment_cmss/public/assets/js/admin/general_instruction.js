$(document).ready(function(){
	function upload_result(){
		var programAdditional = $('#tbluploadresult').dataTable({
			"ajax":
			{
				"url": base_url+"/ajax_controller/get_general_instruction_data",
				"type": "POST",
				"data": ''
			},
			"bPaginate": true,
	...
	$('#frmAdditionalData').bootstrapValidator({
		...
		submitHandler: function(validator, form, submitButton) 
		{
			...
			{
				var action = 'update_instruction_data';
			}
			else
			{
				var action = 'insert_instruction_data';
			}
			formData.append('txtareainstruction', CKEDITOR.instances['txtareainstruction'].getData());
			//ajax call to server
			$.ajax({
...
function edit_additional(event)
{
	...
	CKEDITOR.instances['txtareainstruction'].setData(oTable.fnGetData(row)['instruction']); 
...
function deleteRowAdditional(event)
{
 ...
 function delete_resultdata(){
		...
		//ajax call to server
		$.ajax({
			url:base_url+"ajax_controller/delete_instruction",
			type:"post",
			data:institutedata,
			success:function(response)
			{  
