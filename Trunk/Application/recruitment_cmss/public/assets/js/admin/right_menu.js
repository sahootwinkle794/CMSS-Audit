...
function right_menu(){
	...
	var applicantdetails = $('#tblmenu').dataTable({
	    ...
	    "aoColumns": [    
			...
			{"sName": "default","sWidth": "15%", "sClass":"alignCenter", "sDefaultContent": "<button type='button' id='btnEdit' class='btn btn-warning btn-sm btn-circle tooltipTable' onclick='edit_right_menu(event)' title='Edit' ><i class='fa fa-edit'></i></button>"}
...
$('#menuForm').bootstrapValidator({
	...
	submitHandler: function(validator, form, submitButton) 
	{
		...
		//ajax call to server
		$.ajax({
			url:base_url+"ajax_controller/operation_right_menu",
			type:"post",
			data:formData,
			cache: false,
	        contentType: false,
...
function edit_right_menu(event)
{
	$('#menuForm').data('bootstrapValidator').resetForm(true);
	...
	if(event.target.tagName == "BUTTON" || event.target.tagName == "A")
  	...
  	$('#hidOperType').val('edit_right_menu');
	...
	$('#hidid').val(id);
	$("#myModalLabelHeader").html("Update Right Menu");
}

function add_right_menu(){

	$('#menuForm').data('bootstrapValidator').resetForm(true);
	$("#hidOperType").val("add_right_menu");
	$('#editModal').modal('show');
	$("#myModalLabelHeader").html("Add Right Menu");
}
