...
$('#txtApplDate').datepicker({
	...
	$("#btnChange").click(function(){
		var change_program = $("#cmbChangeProgram").val();
		if(change_program == '')
		{
			toastr.error("Please Select a Program to Change");
		}
		else
		{
			var ques = confirm("Are You Sure you Want to Change the Program");
			if(ques)
			{
				var program_code = $("#cmbProgram").val();
	    		var reg_user_id = $("#txtRegUserId").val();
				var change_program_code = $("#cmbChangeProgram").val();
				var appl_no = $("#spanApplicantNo").html();
				var institutedata = {
					program_code : program_code,
					reg_user_id : reg_user_id,
					change_program_code : change_program_code,
					appl_no : appl_no
				}
				$.ajax({
					url:base_url+"/ajax_controller/get_change_program_admit_card_setup",
					type:"post",
					data : institutedata,
					success:function(response){ 
						var res1 = JSON.parse(response); 
