$(document).ready(function(){
	...
	$("#btnChange").click(function(){
		var change_mobile = $("#txtChangeRegUserId").val();
		
		if(change_mobile == '')
		{
			toastr.error("Please Enter a Mobile No to Change");
		}
		else if(change_mobile.length != 10)
		{
			toastr.error("Please Enter a 10 Digit  mobile No");
		}
		else
		{
			var ques = confirm("Are You Sure you Want to Change the Registered Mobile");
			if(ques)
			{
				var program_code = $("#cmbProgram").val();
	    		var reg_user_id = $("#txtRegUserId").val();
				var change_reg_user_id = $("#txtChangeRegUserId").val();
				var appl_no = $("#spanApplicantNo").html();
				
				var institutedata = {
					program_code : program_code,
					reg_user_id : reg_user_id,
					change_reg_user_id : change_reg_user_id
				}
				$.ajax({
					url:base_url+"/ajax_controller/get_check_mobile_admit_card_setup",
					type:"post",
					data : institutedata,
					success:function(response){ 
						var res1 = JSON.parse(response); 
						if(res1.aaData == '1')
						{
							toastr.error("This Mobile No is Already Registered with this Program. Cannot be Changed");
						}
						else if(res1.aaData == '0')
						{
							var institutedata = {
								program_code : program_code,
								reg_user_id : reg_user_id,
							...
							$.ajax({
								url:base_url+"/ajax_controller/get_change_mobile_admit_card_setup",
								type:"post",
								data : institutedata,
								success:function(response){ 
									var res1 = JSON.parse(response); 
