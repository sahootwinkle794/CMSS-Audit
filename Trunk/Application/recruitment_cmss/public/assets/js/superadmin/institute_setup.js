...
$(document).ready(function()
{
	...
	var institutedetailstable = $('#institutedetails').dataTable({
		...
		"bServerSide": false, //Feature control DataTables' server-side processing mode.
		"bDestroy": true,
		"bPaginate": true,
		"bPaging":   true,
		"bInfo":     true,
	    ...
	    "aoColumns": [    
			...
			{ "sName": "institute_code","sWidth": "10%","bVisible":false},
			{ "sName": "institute_type","sWidth": "15%"},
			{ "sName": "website_address","bVisible":false},
			{ "sName": "contact_number","sWidth": "15%"},
			
			{ "sName": "record_status","bVisible":false },
			
			{ "sName": "logo","sWidth": "10%","sClass":"alignCenter","bVisible":false,
				"mRender": function( data, type, full ) {
			        return '<img src="'+base_url+'public/assets/images/'+ data +'.png?v='+Math.random()+'" ></img>';
			    }  },
			{ "sName": "logo_url", "sClass":"alignCenter",
			   	"mRender": function( data, type, full ) {
			        return '<img src="'+base_url+'upload/image/logo/'+ data +'" style="width:40px;height:40px"></img>';
			    } },
			{ "sName": "location","sWidth": "10%"},
			{ "sName": "Action","sClass": "alignCenter",data:null,"sWidth": "10%", "sDefaultContent": "<button class='btn btn-warning  tooltipTable btn-circle' title='Edit' id = 'institutemanageeditbtn' onclick='instituteRow(event);'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></button>"},
			{ "sName": "program_view_structure","bVisible":false },
	...
	/*$("div.institutebutton").html('<div class="btngroup"><button class="btn btn-success" id="institutemanageeditbtn"><i class="fa fa-edit"></i> Edit</button></div>');
	*/$('#institutemanageaddbtn').click(function()
	...
	$('#instmanageeditformid').bootstrapValidator({
		...
		submitHandler: function(validator, form, submitButton) 
		{
			//alert($("#fileinstitutelogoEdit").files[0].type);
			$("#spanProcessingInstituteEdit").show();
			var formData = new FormData(document.getElementById("instmanageeditformid"));
			if($("#fileinstitutelogoEdit").val() != '')
			{
				var fileUpload = $("#fileinstitutelogoEdit")[0];
	...
	/*$($("#fileinstitutelogoEdit")).on("change", function(e) {
	  	const file = this.files[0];
		const  fileType = file['type'];
		const validImageTypes = ['image/gif', 'image/jpeg', 'image/png'];
		if (!validImageTypes.includes(fileType)) {
			$("#fileinstitutelogoEdit").val('');
		    $('#instmanageeditformid').data('bootstrapValidator').updateStatus('fileinstitutelogoEdit', 'NOT_VALIDATED', null).validateField('fileinstitutelogoEdit');
		}
	})*/
	// Delete Record Save	
	 ...
	 { 
		...
		{ 
			if(iFileSize <= 409600)
			{
			  ...
			  readURLSigEdit(this);
			  document.getElementById("institutemanageeditsave").disabled = false;
			...
			else
			{
				...
				document.getElementById("institutemanageeditsave").disabled = true;
				$('#fileinstitutelogoEdit').val("");
				$('#imageDisplayareaEdit').attr('src','');
				$("#imageDisplayareaEdit").attr('height','0');
				$("#imageDisplayareaEdit").attr('width','0');
			...
			//Initiate the JavaScript Image object.
				...
				//Determine the Height and Width.
					...
					if (height > 400 || width > 400) 
					{
						document.getElementById("signMessageEdit1").innerHTML="Error : File size exceeds 400px X 400px";
						document.getElementById("institutemanageeditsave").disabled = true;
					}
					else
					{
    ...
    $('#instituteeditmodal').on('hidden.bs.modal', function (e) {
 		$('#instmanageeditformid').data('bootstrapValidator').resetForm(true);
			...
			if($("#fileInstituteImage1").val() != '')
			{
				...
				//Initiate the JavaScript Image object.
					...
					//Determine the Height and Width.
						...
						{
							toastr.error("lOGO dimension should be less than or equal to 400px X 400px");
...
//EDIT RECORD WITH VALIDATION
	...
	var examCenterSetuptable = $('#dtblExamCenterSetup').dataTable({
        ...
        "bInfo": true,
        "bAutoWidth":false,    
 		"sDom":"<'row'<'col-xs-4'i><'col-xs-4'l><'col-xs-4'f>r>t<'row'<'col-xs-6' <'row' <'col-xs-8 addbuttonExamCentersetup' >>><'col-xs-6'p>>",
		"aoColumns":[    
	                    { "sName": "sl_no","sWidth": "10%" },
						{ "sName": "institute_code","bVisible":false},
	                    { "sName": "institute_name","sWidth": "50%" },
	...
	$("div.addbuttonExamCentersetup").html('<div class="btngroup"><button id="btnAddexamCentersetup" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i> Add</button>&nbsp;</div>');
	...
	$('#frmExamCenterSetupAdd').bootstrapValidator({
		...
		fields: {
			cmbInstituteforExamAdd: {							//form input type name
				validators: {
	                notEmpty: {
							message: 'Organization must be  required and can\'t be empty',
	...
	//Reloading datatable after assigning datatable
...
function editModalForExamcentersetup(event)
{
	...
	var hidInstituteCode =  oTable.fnGetData( row )['institute_code'];
	//alert(hidInstituteCode);
	$('#hidInstituteCode').val(hidInstituteCode);
...
function instituteRow(event)
{
	$('#instmanageeditformid').data('bootstrapValidator').resetForm(true);
	var oTable = $('#institutedetails').dataTable();
	var row;
	if(event.target.tagName == "BUTTON" || event.target.tagName == "A" || event.target.tagName == "IMG")
	  		row = event.target.parentNode.parentNode;
	else if(event.target.tagName == "I")
	...
	$("#fileInstituteImageEdit").val("");
	$('#instituteeditname').val(oTable.fnGetData(row)['institute_name']);
    $('#instituteeditcode').val(oTable.fnGetData(row)['institute_code']);
    $('#txtinstituteTypeEdit').val(oTable.fnGetData(row)['institute_type']);
	$('#fileinstitutelogoEdit').val("");
    $('#txtWebaddressEdit').val(oTable.fnGetData(row)['website_address']);
    $('#txtContactNoEdit').val(oTable.fnGetData(row)['contact_number']);
    $('#txtLocationEdit').val(oTable.fnGetData(row)['location']);
    $('#cmbRecordStatusEdit').val(oTable.fnGetData(row)['record']);
    $('#cmbProgStructEdit').val(oTable.fnGetData(row)['program_view_structure']);
    $('#instituteeditAddress').val(oTable.fnGetData(row)['institute_address']);
    $('#instituteadmindisplaynameEdit').val(oTable.fnGetData(row)['admin_name']);
    $('#instituteadminusernameEdit').val(oTable.fnGetData(row)['admin_user_name']);
	var inemail = oTable.fnGetData(row)[15];
    $('#txtinstituteEmailEdit').val(inemail);
    //var inemail = oTable.fnGetData(row)['institute_email']
    //alert(inemail);
    var image = oTable.fnGetData(row)['image_url'];
	var institute_image = base_url+"public/assets/images/"+image;
		//alert(signature);
	$('#imageDisplayareaEdit').attr('src', institute_image);
	$("#imageDisplayareaEdit").attr('height','100');
