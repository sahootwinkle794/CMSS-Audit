				...
				$.ajax({
					...
					success:function(response){  
						...
						if(res1.eligibility == '0'){
							swal({
								title: "Sorry",
								text: "You are not eligibleto apply for  the post according to the eligibility criteria",
								type: "error"
							},
							function(isConfirm) {
							  if (isConfirm) {
...
var pincode=$("#txtPresentPin").val();
	$("#spanAddressload").show();
	...
	$.ajax({
		...
		success: function(response){
			...
			{
				var post_office=result.PostOffice;
				//console.log(post_office);
				//var post_office1=result.PostOffice[0];
				var html = '';
				var options = "<option value =''>Select Postoffice</option>";
					for(var i=0;i<post_office.length;i++){
				   // options = options + "<option value='"+result.PostOffice[i].Name+"' >"+result.PostOffice[i].Name+"</option>";
				   	 html += '<option value='+result.PostOffice[i].Name+'>'+result.PostOffice[i].Name+'</option>';
				   	} 
				   	$('#txtPresentPost').html(html);
				 //   	$('#txtPresentPost').html("");   //campusid from academicPeriod
					// $('#txtPresentPost').append(options);
				//console.log(result.PostOffice[0].Block);
				//console.log(result.PostOffice);

				
				$("#city_name").val(result.PostOffice[0].Block);
	            $("#cmbPresentDist").val(result.PostOffice[0].District);
	            $("#cmbPresentState").val(result.PostOffice[0].State);
	            $("#spanAddressload").hide();
			}
			else
			{
...
var pincode=$("#txtPermanentPin").val();
	//data : {id:empId, name:empName, age:empAge, designation:empDesignation, skills:empSkills, address:empAddress},
	$.ajax({
		type : "GET",
		url  : "https://api.postalpincode.in/pincode/"+pincode,
		...
		success: function(response){
			...
			var options = "<option value =''>Select Postoffice</option>";
				for(var i=0;i<post_office.length;i++){
					console.log(post_office[i]['Name']);
			   // options = options + "<option value='"+result.PostOffice[i].Name+"' >"+result.PostOffice[i].Name+"</option>";
			   	 html += '<option value="'+post_office[i]['Name']+'">'+post_office[i]['Name']+'</option>';
			   	} 
			   	console.log(html);
			   	$('#txtPermanentPost').html(html);
			 //   	$('#txtPresentPost').html("");   //campusid from academicPeriod
				// $('#txtPresentPost').append(options);
			//console.log(result.PostOffice[0].Block);
			//console.log(result.PostOffice);
			...
			$("#city_name1").val(result.PostOffice[0].Block);
            $("#cmbPermanentDist").val(result.PostOffice[0].District);
            $("#cmbPermanentState").val(result.PostOffice[0].State);
...
function permanentPostChange(post_name=''){
			...
			$.ajax({
				...
				success:function(response){  
					...
					if(result.Status != 'Error')
					{
						...
						for(var i=0;i<post_office.length;i++){
							var post_office_name=result.PostOffice[i].Name;
								if(post_office_name == post_name){
									selected='selected';
								}else{
									selected='';
								}
							// }
							options = options + "<option value="+post_office_name+" "+selected+">"+post_office_name+"</option>";
							
						}
						$('#txtPermanentPost').html("");   //campusid from academicPeriod
						$('#txtPermanentPost').append(options);
					}
					/*else
					{
						toastr.error("Inavalid");
					}*/
					},
				error:function(){
					toastr.error("We are unable to Process.Please contact Support");
...
$( document ).ready(function() {
    ...
    var permanentpincode=document.getElementById("txtPermanentPin").value;
    ...
    if(permanentpincode != '')
    {
    	...
    	$.ajax({
		type : "GET",
		url  : "https://api.postalpincode.in/pincode/"+permanentpincode,
		dataType : "JSON",
		success: function(response){
			//var res = JSON.parse(response);
			var result= response;
    	...
    	pincode=document.getElementById("txtPresentPin").value;
    	 if(pincode != '')
    	{
    	var prsentpincode=document.getElementById("txtPresentPin").value;

    	$.ajax({
		type : "GET",
		url  : "https://api.postalpincode.in/pincode/"+prsentpincode,
		dataType : "JSON",
		success: function(response){
			//var res = JSON.parse(response);
			...
			var post_office=result1.PostOffice;
			//console.log(post_office);
			//var post_office1=result.PostOffice[0];
			var html = '';
			var options = "<option value =''>Select Postoffice</option>";
				for(var i=0;i<post_office.length;i++){
			   // options = options + "<option value='"+result.PostOffice[i].Name+"' >"+result.PostOffice[i].Name+"</option>";
			   	 html += '<option value='+result1.PostOffice[i].Name+'>'+result1.PostOffice[i].Name+'</option>';
			   	} 
			   	$('#txtPresentPost').html(html);
			 //   	$('#txtPresentPost').html("");   //campusid from academicPeriod
				// $('#txtPresentPost').append(options);
			...
			$("#city_name").val(result1.PostOffice[0].Block);
            $("#cmbPresentDist").val(result1.PostOffice[0].District);
            $("#cmbPresentState").val(result1.PostOffice[0].State);
